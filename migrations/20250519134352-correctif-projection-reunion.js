...
exports.up = async function (db) {
  ...
  await db.runSql(/* sql */ `
    CREATE OR REPLACE FUNCTION public.to_legal_projection(geom geometry)
RETURNS geometry AS $$
...
BEGIN
  RETURN ST_Transform(geom, COALESCE((
    ...
    FROM (
      VALUES
        (2154, ST_MakeEnvelope(-5.1412, 41.334, 9.5597, 51.0888, 4326)),   -- Métropole
        (5490, ST_MakeEnvelope(-61.8098, 14.3947, -60.8106, 16.511, 4326)), -- Antilles
        (2975, ST_MakeEnvelope(55.2166, -21.3891, 55.8366, -20.8721, 4326)), -- Réunion (correction)
        (2972, ST_MakeEnvelope(-54.6023, 2.1111, -51.619, 5.7487, 4326)), -- Guyane (correction)
        (4471, ST_MakeEnvelope(45.0185, -13.0001, 45.298, -12.6366, 4326)) -- Mayotte
      ) AS t(legal_projection, region_bounds)
    WHERE ST_Intersects(ST_Transform(geom, 4326), region_bounds)
    LIMIT 1
  ), 2154));
END;
$$ LANGUAGE plpgsql;
