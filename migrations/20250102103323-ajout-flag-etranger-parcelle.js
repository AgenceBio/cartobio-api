'use strict';

var dbm;
var type;
var seed;

/**
  * We receive the dbmigrate dependency from dbmigrate initially.
  * This enables us to not have to rely on NODE_PATH.
  */
exports.setup = function(options, seedLink) {
  dbm = options.dbmigrate;
  type = dbm.dataType;
  seed = seedLink;
};

exports.up = async function(db) {
  await db.addColumn("cartobio_parcelles", "etranger", {
    type: "boolean",
    defaultValue: false
  });

  await db.createTable('territoires', {
    fid: {
      type: 'int',
      primaryKey: true,
      notNull: true,
      unsigned: true
    },
    geom: 'geometry'
  });

  await db.runSql(
    /* sql */`
    CREATE OR REPLACE FUNCTION update_communes() RETURNS trigger AS $$
    BEGIN
        IF NEW.commune IS NOT NULL OR ST_IsValid(NEW.geometry) = false THEN
          RETURN NEW;
        END IF;

        NEW.etranger = 
          EXISTS(
            SELECT fid
            FROM territoires
            WHERE ST_Intersects(
                NEW.geometry,
                territoires.geom
            )
          ) = false;

        IF NEW.etranger = true THEN
          RETURN NEW;
        END IF;

        NEW.commune = (
          SELECT code
          FROM communes
          ORDER BY ST_Area(
            ST_Intersection(
              NEW.geometry,
              communes.geometry
            )
          ) DESC,
          ST_Distance(
            NEW.geometry,
            communes.geometry
          ) ASC
          LIMIT 1
        )::text;

        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
  `)
};

exports.down = async function(db) {
  await db.removeColumn("cartobio_parcelles", "from_parcelles");
  await db.dropTable('territoires');
  await db.runSql(
    /* sql */`
    CREATE OR REPLACE FUNCTION update_communes() RETURNS trigger AS $$
    BEGIN
        NEW.commune = (SELECT CASE
          WHEN NEW.commune IS NULL AND ST_IsValid(NEW.geometry)
          THEN (
              SELECT code
              FROM communes
              WHERE ST_Intersects(
                  NEW.geometry,
                  communes.geometry
              )
              ORDER BY ST_Area(
                ST_Intersection(
                  NEW.geometry,
                  communes.geometry
                )
              ) DESC
              LIMIT 1
          )::text
          ELSE NEW.commune
          END
        );
        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
  `)
};

exports._meta = {
  "version": 1
};
