...
if (reportErrors) {
  const sentryOptions = {
    ...
    integrations: [Sentry.extraErrorDataIntegration(), Sentry.localVariablesIntegration()],
...
app.register(async (app) => {
  ...
  app.get("/api/v2/stats", internalSchema, async (request, reply) => {
    const [dataGouv, stats] = await Promise.all([getDataGouvStats(), getParcellesStats()]);
  ...
  app.get(
    ...
    (request, reply) => {
      ...
      return fetchUserOperators(userId, limit, offset).then((res) => reply.code(200).send(res));
    }
  );
  ...
  app.post(
    "/api/v2/operator/:numeroBio/records",
    mergeSchemas(createRecordSchema, operatorFromNumeroBio, protectedWithToken()),
    async (request, reply) => {
  ...
  app.delete(
    ...
    mergeSchemas(routeWithRecordId, protectedWithToken()),
    async (request, reply) => {
  ...
  app.patch(
    "/api/v2/audits/:recordId",
    mergeSchemas(protectedWithToken(), patchRecordSchema, routeWithRecordId),
    (request, reply) => {
      const { body: patch, user, record } = request;

      return updateAuditRecordState({ user, record }, patch).then((record) =>
        reply.code(200).send(normalizeRecord(record))
      );
    }
  );
  ...
  app.post(
    ...
    mergeSchemas(protectedWithToken(), createFeatureSchema, routeWithRecordId),
    (request, reply) => {
      const { feature } = request.body;
      const { user, record } = request;

      return addRecordFeature({ user, record }, feature).then((record) =>
        reply.code(200).send(normalizeRecord(record))
      );
    }
  );
  ...
  app.patch(
    "/api/v2/audits/:recordId/parcelles",
    mergeSchemas(protectedWithToken(), patchFeatureCollectionSchema, routeWithRecordId),
    (request, reply) => {
      const { body: featureCollection, user, record } = request;

      return patchFeatureCollection({ user, record }, featureCollection.features).then((record) =>
        reply.code(200).send(normalizeRecord(record))
      );
    }
  );
  ...
  app.patch(
    ...
    mergeSchemas(protectedWithToken(), updateFeaturePropertiesSchema, routeWithRecordId),
    (request, reply) => {
      const { body: feature, user, record } = request;
      const { featureId } = request.params;

      return updateFeature({ featureId, user, record }, feature).then((record) =>
        reply.code(200).send(normalizeRecord(record))
      );
    }
  );
  ...
  app.delete(
    "/api/v2/audits/:recordId/parcelles/:featureId",
    mergeSchemas(protectedWithToken(), deleteSingleFeatureSchema, routeWithRecordId),
    (request, reply) => {
      const { user, record } = request;
      const { reason } = request.body;
      const { featureId } = request.params;

      return deleteSingleFeature({ featureId, user, record }, { reason }).then((record) =>
        reply.code(200).send(normalizeRecord(record))
      );
    }
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      return parseTelepacArchive(request.file()).then((geojson) => reply.send(geojson));
    }
  );
  ...
  app.post("/api/v2/convert/geofolia/geojson", mergeSchemas(protectedWithToken()), async (request, reply) => {
    ...
    return parseGeofoliaArchive(await data.toBuffer()).then((geojson) => reply.send(geojson));
  });
  ...
  app.post("/api/v2/convert/anygeo/geojson", mergeSchemas(protectedWithToken()), async (request, reply) => {
    return parseAnyGeographicalArchive(request.file()).then((geojson) => reply.send(geojson));
  });
  ...
  app.get(
    ...
    mergeSchemas(protectedWithToken({ cartobio: true, oc: true }), routeWithPacage),
    async (request, reply) => {
      const { numeroPacage } = request.params;

      return pacageLookup({ numeroPacage }).then((featureCollection) => reply.send(featureCollection));
    }
  );
  ...
  app.head(
    ...
    mergeSchemas(protectedWithToken({ cartobio: true, oc: true }), operatorFromNumeroBio, geofoliaImportSchema),
    async (request, reply) => {
  ...
  app.get(
    "/api/v2/import/geofolia/:numeroBio",
    mergeSchemas(protectedWithToken({ cartobio: true, oc: true }), operatorFromNumeroBio),
    async (request, reply) => {
      const { siret } = request.operator;
  ...
  app.post(
    "/api/v2/certification/parcelles",
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const records = iterateOperatorLastRecords(request.organismeCertificateur.id, {
  ...
  app.get(
    "/api/v2/user/verify",
    mergeSchemas(protectedWithToken({ oc: true, cartobio: true }), sandboxSchema, internalSchema),
    (request, reply) => {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      return reply.redirect(
        `${config.get("frontendUrl")}/login?mode=${mode}&returnto=${returnto}#token=${cartobioToken}`
      );
    }
  );
});
