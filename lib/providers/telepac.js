...
const { LegalProjections } = require('../enums')
...
async function fromShapefileArchive (stream) {
  ...
  try {
    ...
    // @ts-ignore
    const geojsonString = await pipeline(
      stream,
      geo.from('shp'),
      geo.to('geojson'),
      getStream
    ...
    throw new InvalidRequestApiError('Il ne s\'agit pas d\'un fichier Telepac "Parcelles déclarées" ou "Parcelles instruites".', { cause: error })
  }
  ...
  return Object.entries(regions).map(([region, regionGeojson]) => {
    const projection = LegalProjections[region]
