...
async function fromShapefileArchive (data) {
  ...
  const geojson = await stream.catch(error => {
    throw new InvalidRequestApiError('Impossible de trouver l\'archive Telepac dans ce fichier.', error)
  })

  const featureCollection = {
    type: 'FeatureCollection',
    features: JSON.parse(geojson).features.map(({ properties, geometry, type }) => {
      const id = getRandomFeatureId()

      const { AGRIBIO, CAMPAGNE, COMMUNE, NUMERO_I, NUMERO_P, PACAGE, TYPE } = properties
