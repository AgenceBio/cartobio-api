 ...
 * @param {Array<Object>} errors - Liste détaillée des erreurs rencontrées.
 ...
 *
 */
async function updateJobError (numeroBioValid, numeroBioError, count, errors, warning, jobId) {
  const importResult = await pool.query(
    `
    UPDATE parcellaire_import SET
    nb_objets_acceptes = $1,
    ...
    [
      ...
      numeroBioError.length,
      count,
      JSON.stringify({ count, errors: errors, warning, numeroBioValid, numeroBioError: numeroBioError.map(e => e.numeroBio) }),
      'DONE',
      jobId
    ]
  )
