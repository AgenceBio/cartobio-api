...
async function * preparseAPIParcellaireStream (stream) {
  ...
  for await (const record of streamRecords) {
    ...
    const numeroClient = String(record.numeroClient)
...
async function parcellaireStreamToDb (stream, organismeCertificateur, jobId) {
  ...
  const importResult = await pool.query(
    ...
    RETURNING id`,
    [
      JSON.stringify(numeroBioValid),
      JSON.stringify(numeroBioError),
...
async function getCurrentStatusJobs (id) {
  const result = await pool.query(
    'SELECT status, result_job, ended_at, created_at from parcellaire_import where id = $1',
    [id]
  )
...
/**
 * @param {'error' | 'warning' | 'all' | 'none' | null | undefined} logs
 * @returns {string}
 */
function buildLogsSubquery (logs) {
  if (!logs || logs === 'none') return ', NULL AS logs'

  const logsCondition = logs === 'error'
    ? 'AND pil.type = \'error\''
    : logs === 'warning'
      ? 'AND pil.type = \'warning\''
      : ''
  ...
  return `, (
    SELECT json_agg(json_build_object(
      ...
      'type', pil.type,
      'message', pil.message,
      'numero_bio', pil.numero_bio
    ))
    FROM parcellaire_import_logs pil
    WHERE pil.import_id = pi.id ${logsCondition}
  ) AS logs`
}

/**
 * @param {{ status?: string, organismeCertificateur?: string, from?: string, to?: string, withPayload?: string, logs?: 'error' | 'warning' | 'all' | 'none' | null, page: string | number, limit: string | number }} params
 * @returns {Promise<{ data: object[], meta: { total: number, page: number, limit: number } }>}
 */
async function getImportList ({ status, from, to, withPayload, logs, page, limit }) {
  const conditions = []
  const params = []
  let idx = 1
  ...
  const payloadSelect = withPayload === 'true'
    ? ', json_build_object(\'id\', pip.id, \'payload\', pip.payload) AS payload'
    : ', NULL AS payload'
  const payloadJoin = withPayload === 'true'
...
/**
 * @param {{ id: string | number, withPayload?: string, logs?: 'error' | 'warning' | 'all' | 'none' | null }} params
 * @returns {Promise<object | null>}
 */
async function getImportById ({ id, withPayload, logs }) {
  const payloadSelect = withPayload === 'true'
    ? ', json_build_object(\'id\', pip.id, \'payload\', pip.payload) AS payload'
    : ', NULL AS payload'
  const payloadJoin = withPayload === 'true'
...
async function getImportPayload ({ id }) {
  const { rows } = await pool.query(
    'SELECT id, payload FROM parcellaire_import_payload WHERE import_id = $1',
    [id]
  )
