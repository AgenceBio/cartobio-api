...
function formatParcelleName (parcelle) {
  ...
  if (parcelle.nom) return `${parcelle.nom}`
  if (ilot && num) return `Îlot ${ilot} parcelle ${num}`
  if (ilot) return `Îlot ${ilot}`
  if (num) return `Parcelle ${num}`
  return '-'
}
...
async function * parseAPIParcellaireStream (stream, { organismeCertificateur }) {
   ...
   */
  const payloads = stream?.[Symbol.asyncIterator] || stream?.[Symbol.iterator]
    ? stream
    : [stream]
  const streamRecords = (async function * () {
    for await (const payload of payloads) {
      if (Array.isArray(payload)) yield * payload
      else yield payload
    }
  })()
  ...
  for await (const record of streamRecords) {
      ...
      record.parcelles.map(async (parcelle) => {
        ...
        try {
          ...
          featureErrors.push(
            buildFeatureError(
              parcelle,
              'champ geom incorrect : ' + String(message),
    ...
    yield {
      record: {
        ...
        parcelles: /** @type {CartoBioFeatureCollection} */ (featureCollection(features)),
...
async function collectFullValidationResults (stream, { organismeCertificateur }, jobId) {
  ...
  try {
    ...
    for await (const {
      ...
      if (!statusUpdated && json) {
        await updateStatus(jobId, json)
        statusUpdated = true
      }
  ...
  try {
    for (const { record, numeroBio, warnings } of validItems) {
      ...
      try {
        ...
        const apiError = /** @type {{ code?: string, message?: string }} */ (e)
        if (apiError.code === 'INVALID_API_REQUEST' || apiError.code === 'MISSING_GEOMETRY') {
          dbErrors.push({
            numeroBio: record.numerobio,
            message: apiError.message || 'Erreur de validation lors de l’import.',
            code: apiError.code === 'MISSING_GEOMETRY' ? ImportLogType.MISSING_GEOM : ImportLogType.DB_ERROR
          })
...
async function addPayload (json, jobId) {
  ...
  const [firstRecord] = json

  await pool.query(
    'UPDATE parcellaire_import SET numerobio=$1, numeroclient=$2, audit_date=$3 WHERE id=$4',
    [
      firstRecord?.numeroBio ? String(firstRecord.numeroBio) : null,
      firstRecord?.numeroClient ? String(firstRecord.numeroClient) : null,
      firstRecord?.dateAudit && !isNaN(Date.parse(firstRecord.dateAudit)) ? new Date(firstRecord.dateAudit).toISOString() : null,
      jobId
    ]
  )
 ...
 * @returns {Promise<{ status: string, numeroBio?: string, numeroClient?: string, auditDate?: Date, result?: any, error?: string, ended?: Date, created?: Date }>} - Statut du job.
 */
async function getCurrentStatusJobs (id) {
  const result = await pool.query(
    'SELECT status, numerobio, numeroclient, audit_date, result_job, ended_at, created_at from parcellaire_import where id = $1',
    [id]
  )
  ...
  if (job.status === 'DONE') {
    return {
      status: job.status,
      numeroBio: job.numerobio,
      numeroClient: job.numeroclient,
      auditDate: job.audit_date,
...
async function processFullJob (jobId, validItems, errors) {
  ...
  try {
    const result = await parcellaireValidItemsToDb(validItems, errors, jobId)
    const statutJob = result.errors.length > 0
      ? 'REJECTED'
      : 'VALID'
    await pool.query(
    `
      UPDATE parcellaire_import
      SET statut_job = $1
      WHERE id = $2
    `,
    [statutJob, jobId]
    ...
    await pool.query(
      `
        ...
        SET
          status = 'ERROR',
          statut_job = 'ERROR'::result_job_enum,
          result_job = $1,
          ended_at = NOW()
        WHERE id = $2
      `,
...
/**
 * Renseigne les métadonnées du job depuis le premier record reçu et détermine
 * s'il s'agit d'une création ou de la mise à jour d'un import existant.
 *
 * @async
 * @param {number} jobId - Identifiant du job à mettre à jour.
 * @param {import('../outputs/types/api').InputApiRecord} record - Premier record du flux.
 * @returns {Promise<void>}
 */
async function updateStatus (jobId, record) {
  const numeroBio = record?.numeroBio != null
    ? String(record.numeroBio)
    : null

  const numeroClient = record?.numeroClient != null
    ? String(record.numeroClient)
    : null

  const auditDate = record?.dateAudit && !Number.isNaN(Date.parse(record.dateAudit))
    ? new Date(record.dateAudit).toISOString().slice(0, 10)
    : null

  await pool.query(
      `
        UPDATE parcellaire_import AS current_job
        SET
          numerobio = $1::text,
          numeroclient = $2::text,
          audit_date = $3::date,
          etat = CASE
            WHEN $1::text IS NULL
              OR $2::text IS NULL
              OR $3::date IS NULL
              THEN 'UNKNOWN'::etat_enum

            WHEN EXISTS (
              SELECT 1
              FROM parcellaire_import AS previous_job
              WHERE previous_job.id <> current_job.id
                AND previous_job.numerobio = $1::text
                AND previous_job.numeroclient = $2::text
                AND previous_job.audit_date = $3::date
            )
              THEN 'UPDATED'::etat_enum

            ELSE 'CREATED'::etat_enum
          END
        WHERE current_job.id = $4::integer
      `,
      [numeroBio, numeroClient, auditDate, jobId])
}
module.exports = {
  ImportLogType,
