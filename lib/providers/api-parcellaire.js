      ...
      record.parcelles.map(async (parcelle) => {
        ...
        if (!Number.isNaN(numeroIlot) && numeroIlot !== 0 && parcelle.enAttentePac === true) {
          parcelleWarnings.push({
            numeroBio: String(record.numeroBio),
            message: `Parcelle ${id ?? ''} acceptée mais ne peut être marquée comme en attente PAC car un îlot est renseigné`,
...
/**
 * @param {{ status?: string, organismeCertificateur?: string, from?: string, to?: string, payload?: string, withRejected?: boolean, logs?: 'error' | 'warning' | 'all' | 'none' | null, page: string , limit: string }} params
 * @returns {Promise<{ data: object[], meta: { total: number, page: number, limit: number } }>}
 */
async function getImportList ({ status, from, to, payload, withRejected, page, limit }) {
  const conditions = []
  const params = []
  let idx = 1
