  ...
  for await (const record of streamRecords) {
    ...
    if (
      ...
      yield {
        numeroBio: String(record.numeroBio),
        error: new Error(
          'Numéro client différent -> numéro attendu  : ' +
            operator.organismeCertificateur.numeroClient
        )
      }
      continue
  ...
  for await (const record of streamRecords) {
    ...
    if (
      ...
      yield {
        numeroBio,
        error: new Error(
          `Numéro client différent -> numéro attendu : ${operator.organismeCertificateur.numeroClient}`
        )
      }
      index++
      continue
...
async function parcellaireStreamToDb (stream, organismeCertificateur) {
  ...
  const importResult = await pool.query(
    `INSERT INTO parcellaire_imports
     ...
     RETURNING id`,
    [
      String(organismeCertificateur.id),
...
async function getCurrentStatusJobs (id) {
  const result = await pool.query(
    'SELECT status, "result", ended, created from jobs_import where id = $1',
    [id]
  )
