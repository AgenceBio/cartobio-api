const { deleteSingleFeature, createOrUpdateOperatorRecord, updateAuditRecordState, updateFeature, searchControlBodyRecords } = require('./cartobio.js')
...
describe('test search with filter', () => {
  ...
  test('filter on etatCertification', async () => {
    ...
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { etatCertification: 'CERTIFIED', anneeReference: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 2 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { etatCertification: 'NO_CERTIFIED', anneeReference: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 2, total: 8 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { etatCertification: 'CERTIFIED', anneeReference: 1998 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })
  })
