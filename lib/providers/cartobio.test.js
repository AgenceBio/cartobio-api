...
describe('test search with filter', () => {
  beforeEach(async () => {
    ...
    get.mockReturnValueOnce({
      async json () {
        return agencebioOperatorsApiResults
      }
    })
  })

  test('no filter, recors with no notifications filtered', async () => {
  ...
  test('no filter, recors with no notifications filtered, page 2', async () => {
  ...
  test('filter on etatCertification', async () => {
    ...
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { etatCertification: 'NO_CERTIFIED', anneeReferenceCertification: new Date().getFullYear() } })
  ...
  test('filter on statutParcellaire', async () => {
    ...
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['NONE'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 6 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['NONE'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 6 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['PENDING_CERTIFICATION'] } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 1 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['PENDING_CERTIFICATION'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 1 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['PENDING_CERTIFICATION'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['OPERATOR_DRAFT'] } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 1 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['OPERATOR_DRAFT'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 1 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['OPERATOR_DRAFT'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['AUDITED'] } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['AUDITED'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['AUDITED'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED'] } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 2 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 2 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })

    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED', 'AUDITED'] } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 2 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED', 'AUDITED'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 2 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED', 'AUDITED'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })
    ...
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED', 'AUDITED', 'PENDING_CERTIFICATION', 'OPERATOR_DRAFT'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 4 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['CERTIFIED', 'AUDITED', 'PENDING_CERTIFICATION', 'OPERATOR_DRAFT'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 0 })
    ...
    expect(res.pagination).toEqual({ page: 1, page_max: 2, total: 10 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['NONE', 'CERTIFIED', 'AUDITED', 'PENDING_CERTIFICATION', 'OPERATOR_DRAFT'], anneeReferenceStatut: new Date().getFullYear() } })
    expect(res.pagination).toEqual({ page: 1, page_max: 2, total: 10 })
    res = await searchControlBodyRecords({ ocId: 1, userId: 1, input: '', page: 1, filter: { statutParcellaire: ['NONE', 'CERTIFIED', 'AUDITED', 'PENDING_CERTIFICATION', 'OPERATOR_DRAFT'], anneeReferenceStatut: 2001 } })
    expect(res.pagination).toEqual({ page: 1, page_max: 1, total: 6 })
  })
