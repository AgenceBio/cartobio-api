...
describe("getParcellesStats", () => {
  ...
  test("query returns stats count", async () => {
    return expect(await getParcellesStats("ccc" /* worksaround memoization */)).toMatchObject({
      parcellaires: "4",
      parcelles: "12",
    });
  });
...
describe("deleteSingleFeature", () => {
  ...
  test("throws an error when trying to remove a non-existing feature", async () => {
    await expect(deleteSingleFeature({ featureId: 9999, record: normalizedRecord, user }, { reason })).rejects.toThrow(
      NotFoundApiError
    );
...
describe("evvLookup", () => {
  ...
  test("remote server down", () => {
    expect.assertions(1);
    evvClient.get.mockReturnValueOnce({
      text: jest.fn().mockRejectedValue(new TimeoutError({ message: "Request timeout" })),
    });
