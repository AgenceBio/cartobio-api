const { deleteSingleFeature, getDataGouvStats, getParcellesStats, createOrUpdateOperatorRecord } = require('./cartobio.js')
...
describe('parseAPIParcellaireStream', () => {
  test('turns a file into a working GeoJSON', async () => {
    ...
    await expect(generator.next()).resolves.toMatchObject({
      value: {
        numeroBio: '123456',
        error: new Error('geometry en dehors des régions autorisées.')
      }
    })
