...
all_changes AS (
  SELECT geometry, status, new_id, old_id FROM modified
  UNION ALL
  SELECT geometry, status, NULL as new_id, old_id FROM deleted
  UNION ALL
  SELECT geometry, status, new_id, NULL as old_id FROM added
)
SELECT
    ...
    ELSE
      jsonb_build_object(
        ...
        'features', jsonb_agg(
          jsonb_build_object(
            'type', 'Feature',
            'properties', jsonb_build_object('status', ac.status,'new_id',ac.new_id, 'old_id', ac.old_id),
