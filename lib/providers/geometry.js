...
deleted AS (
  ...
  WHERE NOT EXISTS (
    SELECT 1
    FROM new n
    WHERE ST_IsValid(o.geometry)
      AND ST_IsValid(n.geometry)
      AND ST_Intersects(n.geometry, o.geometry)
      AND ST_Area(ST_Intersection(n.geometry, o.geometry)) / ST_Area(o.geometry) > 0.1
  )
),
added AS (
  SELECT
  ...
  WHERE NOT EXISTS (
    ...
    WHERE ST_IsValid(o.geometry)
      AND ST_IsValid(n.geometry)
      AND ST_Intersects(o.geometry, n.geometry)
      AND ST_Area(ST_Intersection(o.geometry, n.geometry)) / ST_Area(n.geometry) > 0.1
  )
),
modified AS (
  SELECT
    ...
    o.id as old_id
  FROM old o
  JOIN new n ON ST_IsValid(o.geometry)
    AND ST_IsValid(n.geometry)
    AND ST_Intersects(o.geometry, n.geometry)
    -- Imprésicion
    AND ST_Area(ST_Intersection(o.geometry, n.geometry)) > 0.00000001
  WHERE NOT ST_Equals(
    ST_ReducePrecision(n.geometry, 0.0001),
    ST_ReducePrecision(o.geometry, 0.0001)
  )
),
all_changes AS (
