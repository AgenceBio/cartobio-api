 ...
 * @param {boolean} pac - Boolean permettant de savoir si on doit récuperer uniquement les parcelles PAC (numéro ilot et parcelle)
 * @returns {Promise<Array>} - Tableau des parcelles
 */
async function getAllParcelles (recordId, pac = false) {
  try {
    ...
    SELECT
     ...
     cp.commune,
     COALESCE(c.nom,'inconnu') as communename,
     cp.created,
     cp.engagement_date,
     ...
     COALESCE(SUM(ST_Area(to_legal_projection(cp.geometry)) / 10000), 0) AS superficie_totale_ha
      ...
      LEFT JOIN communes c ON c.code = commune
      WHERE record_id = $1 AND cp.deleted_at IS NULL
      ${pac ? " AND ((cp.numero_ilot_pac IS NOT NULL AND cp.numero_parcelle_pac IS NOT NULL AND cp.numero_parcelle_pac <> '0' AND cp.numero_ilot_pac <> '0') OR cp.attente_pac = TRUE)" : ' '}
...
async function drawAllParcellesOnMap (page, parcelles, { width, height, center, zoom, timeout }) {
  ...
  const buffer = await page.screenshot({ type: 'jpeg', encoding: 'base64', quality: 50 })
  return { buffer }
}
...
async function mergeAndAddFooter (pdfPaths, name, deleteFilesAfter) {
