...
async function * generatePDF (numeroBio, recordId, force = false, pac = false, zip = false) {
  ...
  if (parcelles.length === 0) {
    ...
    throw new Error("Aucune parcelle, impossible de générer l'attestation du parcellaire")
  }
  ...
  if (parcelles.some((p) => {
    ...
    throw new Error("Culture manquante, impossible de générer l'attestation du parcellaire")
  }
  ...
  if (parcelles.some(p =>
    ...
    (p.nbp == null || p.nbp === '') &&
    (p.refcad == null || (Array.isArray(p.refcad) && p.refcad.every((/** @type {string} */ r) => r === '' || r == null)))
  )) {
    ...
    throw new Error("Nom manquant de parcelle, impossible de générer l'attestation du parcellaire")
  }
  ...
  try {
    ...
    savedPaths.forEach((p, index) => {
      let newName
      if (index === 0) newName = `cartobio_attestation_PAC_${currentOperator.annee_reference_controle}_${currentOperator.numeroBio}.pdf`
      else if (index === 1) newName = `cartobio_liste_PAC_${currentOperator.annee_reference_controle}_${currentOperator.numeroBio}.pdf`
      else newName = `fichier_${index}.pdf`
    ...
    console.error(e)
    throw new Error("Une erreur s'est produite, impossible de générer l'attestation du parcellaire")
  }
}
