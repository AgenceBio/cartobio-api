const fs = require('fs')
const { formatDate, getOptimalZoom, calculateGlobalBoundingBox, launchStyleTab, drawParcelleHighlightOnMap, drawAllParcellesOnMap, mergeAndAddFooter, getAllParcelles, imageToBase64 } = require('./utils-pdf')
...
async function generatePDF (numeroBio, recordId) {
  ...
  const blocksHTML = Object.keys(sortedGroupedParcels).map(groupKey =>
    ...
    <div class="block">
      <div class="container">
          <h3 class="header">${item.name ? 'Parcelle ' + item.name : ''}${item.name && item.nbilot ? ' - ' : ''}${item.nbilot ? 'Ilot ' + item.nbilot + ' parcelle ' + item.nbp : ''}${item.name == null && item.nbilot == null && item.refcad ? item.refcad[0] : ''}</h3>
          <div class="info">
