  ...
  engagement_date: {
    fn (order) {
      ...
      return function sortByEngagementDate (a, b) {
        const aNotif = getNotification(a)
        const bNotif = getNotification(b)
  ...
  notifications: {
    fn (order) {
      ...
      return function sortByNotifications (a, b) {
        const aNotif = getNotification(a)?.etatCertification ?? 'BROUILLON'
        const bNotif = getNotification(b)?.etatCertification ?? 'BROUILLON'
  ...
  nom: {
    fn (order) {
      ...
      return function sortByName (a, b) {
        return collator.compare(a.nom || a.denominationCourante || '', b.nom || b.denominationCourante || '') * (order === 'asc' ? SORT.ASCENDING : SORT.DESCENDING)
      }
    },
    psql () {
      // we are not supposed to enter this case
