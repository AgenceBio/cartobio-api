...
const { addRecordData, applyOperatorTextSearch, filterOperatorForAutocomplete } = require('../outputs/operator.js')
...
async function exportDataOcId (ocId) {
  try {
    const { rows } = await pool.query(
      ...
      WHERE co.oc_id = $1
      GROUP BY co.record_id
      ORDER BY CASE WHEN co.numerobio ~E'^\\d+$' THEN co.numerobio::bigint ELSE 0 END ASC, co.created_at DESC;
      `,
      [ocId]
    )
    ...
    for (const row of rows) {
      const operator = operators.find((o) => o.numeroBio === row.numerobio)
