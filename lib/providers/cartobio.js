    ...
    const features = await Promise.all(record.parcelles
      ...
      .map(async (parcelle) => {
        ...
        try {
          ...
          // Il faut que la date d'engagement soit obligatoire pour les parcelles en conversion(CONV), soit en C1, C2 ou C3. (Donc différent de AB)
          if (conversionNiveau !== null && [EtatProduction.C0, EtatProduction.C1, EtatProduction.C2, EtatProduction.C3, EtatProduction.CONV].includes(conversionNiveau) && !Date.parse(parcelle.dateEngagement)) {
