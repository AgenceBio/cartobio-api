  ...
  try {
    result = await client.query(
      /* sql */`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, CASE WHEN $7='' THEN null ELSE $7::date END, CASE WHEN $8='' THEN null ELSE $8::date END, jsonb_build_array($9::jsonb), $10, $11,
                COALESCE($12, 'Version crée le ' || to_char(now(), 'DD/MM/YYYY')))
        ON CONFLICT (numerobio, audit_date)
            DO UPDATE
            SET (oc_id, oc_label, updated_at, metadata, certification_state, certification_date_debut, certification_date_fin, audit_history, audit_date, audit_notes, version_name)
                    = ($2, $3, $4, $5, coalesce($6, cartobio_operators.certification_state), CASE WHEN $7='' THEN null ELSE coalesce($7::date, cartobio_operators.certification_date_debut) END, CASE WHEN $8='' THEN null ELSE coalesce($8::date, cartobio_operators.certification_date_fin) END, (cartobio_operators.audit_history || coalesce($9, '[]')::jsonb), coalesce($10::date, cartobio_operators.audit_date), coalesce($11, cartobio_operators.audit_notes), coalesce($12, cartobio_operators.version_name))
        ...
        const { rows: partialUpdateRows } = await client.query(
          ...
          UPDATE cartobio_parcelles
              ...
              (coalesce($3, cartobio_parcelles.commune), coalesce($4, cartobio_parcelles.cultures),
               coalesce($5, cartobio_parcelles.conversion_niveau), CASE WHEN $6='' THEN null ELSE coalesce($6::date, cartobio_parcelles.engagement_date) END,
      ...
      const { rows } = await client.query(
        /* sql */`
          ...
          VALUES ($1, $2, $3, $4, coalesce((SELECT cultures FROM merged_cultures), $5, '[]'::jsonb),
                  ...
                  CASE WHEN $7='' THEN null ELSE coalesce($7::date, (SELECT engagement_date FROM previous_version)) END, $8,
              ...
              SET (geometry, commune, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
                   ...
                   reference_cadastre) =
                      (coalesce($3, cartobio_parcelles.geometry), coalesce($4, cartobio_parcelles.commune),
                       ...
                       CASE WHEN $7='' THEN null ELSE coalesce($7::date, cartobio_parcelles.engagement_date) END, coalesce($8, cartobio_parcelles.commentaire),
...
async function getRecords (numeroBio) {
  const { rows } = await pool.query(
    /* sql */`
    SELECT ${recordFields}, COUNT(cp.id) as parcelles, SUM(ST_Area(cp.geometry::geography)) as surface
    FROM cartobio_operators
...
async function getOperatorLastRecord (numeroBio, { anneeAudit = null, statut = null } = {}) {
  ...
  const result = await pool.query(/* sql */`
    SELECT ${recordFields}
    FROM cartobio_operators
    WHERE numerobio = $1 ${conditions.length ? `AND ${conditions.join(' AND ')}` : ''}
    ORDER BY updated_at DESC
    LIMIT 1`,
  [
    numeroBio,
  ...
  // pagination is database-based
    ...
    return Promise.all(
      allNumerobioResults
        ...
        .map(numerobio => fetchOperatorByNumeroBio(numerobio))
    )
      ...
      /* sql */`
        SELECT
          *,
          ...
          -- Trying with "cylindrical equal area" https://epsg.io/53034
          --CASE WHEN ST_IsValid(geometry) THEN ST_Area(ST_Transform(geometry, 53034)) ELSE NULL END AS surface
        FROM cartobio_parcelles
        WHERE record_id = $1
