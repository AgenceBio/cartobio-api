...
const { EtatProduction, fetchOperatorById, normalizeEtatProduction, parsePacDetailsFromComment, fetchOperatorByNumeroBio } = require('./agence-bio.js')
 ...
 * @typedef {import('./agence-bio.js').OrganismeCertificateur} OrganismeCertificateur
 */
 ...
 * @param {Object} record
 * @param {String} record.numeroBio
 * @param {Integer} record.ocId
 * @param {String} record.ocLabel
 * @param {Object} record.metadata
 * @param {import('@turf/helpers').FeatureCollection} record.geojson
 * @param {String} [record.certificationState]
 * @param {Object} [context]
 * @param {Object} [context.decodedToken]
 * @param {OperatorRecord} [context.oldRecord]
 * @param {Date} [context.date]
 * @param {import('pg').Client} [client]
 * @returns {OperatorRecord}
 */
async function createOrReplaceOperatorRecord (operatorId, record, context, client) {
  const { decodedToken, oldRecord } = context || {}
  const { numeroBio, ocId, ocLabel, metadata, geojson } = record

  const certificationState = record.certificationState || CertificationState.OPERATOR_DRAFT
  ...
  try {
    ...
    * si on tente d'insèrer un opérateur avec un nouveau numerobio, l'operator_id doit être vide
    ...
    */
    result = await (client || pool).query(/* sql */
      ` INSERT INTO cartobio_operators
        ...
        ON CONFLICT (numerobio)
            WHERE operator_id = $1 OR $1 IS NULL
            DO UPDATE
            SET (oc_id, oc_label, updated_at, metadata, parcelles, certification_state, audit_history)
               = ($3, $4, $5, $6, $7, $8, (cartobio_operators.audit_history || coalesce($9, '[]')::jsonb))
 ...
 *
 * @param {{ recordId: string, featureId: number, decodedToken: Object}} identifiers
 * @param {{ reason: {code: String, details=: String}}} feature
 * @returns {OperatorRecord}
 */
...
async function getOperatorById (operatorId) {
  ...
  let [record] = result.value.rows

  if (!record && operator.status === 'rejected') {
    return null
  }

  if (!record && operator.value.numeroBio) {
    console.log(operator.value.numeroBio)
    const result2 = await pool.query(`
        UPDATE cartobio_operators
        SET operator_id = $1
        WHERE numerobio = $2
        RETURNING ${recordFields}
    `, [operatorId, operator.value.numeroBio])
    record = result2.rows.at(0)
  }
 ...
 * @param {OrganismeCertificateur} stream
...
async function parcellaireStreamToDb (stream, organismeCertificateur) {
  ...
  try {
    for await (const [numeroBio, data, error] of generator) {
      ...
      await createOrReplaceOperatorRecord(null, {
        numeroBio,
        ocId: data.ocId,
