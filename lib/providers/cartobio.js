...
const { normalizeRecord, normalizeRecordSummary, EtatProduction, CertificationState, normalizeEtatProduction } = require('../outputs/record.js')
...
const recordFields = /* sqlFragment */`cartobio_operators.record_id, numerobio, version_name, certification_date_debut, certification_date_fin, certification_state, created_at, updated_at, metadata, audit_date, audit_history, audit_notes, audit_demandes`
...
async function createOrUpdateOperatorRecord (data, context, customClient) {
  ...
  const { numeroBio, ocId, ocLabel, metadata, geojson, versionName } = data
  ...
  try {
    result = await client.query(
      /* sql */`
        ...
        (numerobio, oc_id, oc_label, created_at, metadata, certification_state, audit_history, audit_date, version_name)
        VALUES ($1, $2, $3, $4, $5, $6, jsonb_build_array($7::jsonb), $8,
                COALESCE($9, 'Version crée le ' || to_char(now(), 'DD/MM/YYYY')))
...
async function getRecords (numeroBio) {
  const { rows } = await pool.query(
    ...
    ORDER BY EXTRACT(year from created_at) DESC, updated_at DESC`,
    [numeroBio]
  )
...
async function getOperatorLastRecord (numeroBio, { anneeAudit = null, statut = null } = {}) {
  const result = await pool.query(/* sql */`
    ...
    FROM cartobio_operators
    WHERE numerobio = $1
      ${anneeAudit ? 'AND EXTRACT(year FROM audit_date) = $2' : ''}
      ${statut ? 'AND certification_state = $3' : ''}
    ORDER BY updated_at DESC LIMIT 1`, [numeroBio, anneeAudit, statut])
