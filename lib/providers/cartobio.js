      ...
      if (filter) {
        const temporisation = records.filter(item => {
          ...
          if (filter.statutParcellaire &&
            ...
            (!filter.statutParcellaire.includes(item.certification_state ?? 'NONE') ||
            (item.certification_state && filter.anneeReferenceStatut && filter.anneeReferenceStatut !== item.annee_reference_controle))
          ) {
            return false
          }
