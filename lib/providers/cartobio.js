...
const { parsePacDetailsFromComment, fetchOperatorByNumeroBio, fetchCustomersByOc, fetchCustomersByOcWithRecords } = require('./agence-bio.js')
    ...
    result = await client.query(
      /* sql */`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, nullif($7, '')::date, nullif($8, '')::date, jsonb_build_array($9::jsonb),
                nullif($10, '')::date, $11, COALESCE($12, 'Version créée le ' || to_char(now(), 'DD/MM/YYYY')), COALESCE(nullif($13, '')::int, DATE_PART('year', NOW())))
...
async function searchControlBodyRecords ({ ocId, userId, input, page, sort, order, filter }) {
  ...
  const numeroBioPinned = await getPinnedOperators(userId)

  const records = await fetchCustomersByOcWithRecords({ siret, ocId, numeroBio, nom })
    .then((records) => {
      if (filter) {
        const temporisation = records.filter(item => {
          if (item.notifications === null || item.notifications === undefined) return false
          if (filter.etatCertification != null && filter.etatCertification !== 'ALL') {
            if (item.annee_reference_controle !== filter.anneeReference) return false
          }
          if (filter.statutParcellaire.length > 0 && !filter.statutParcellaire.includes(item.certification_state ?? null)) return false

          if (filter.departement.length > 0) {
            if (!filter.departement.includes(item.departement)) return false
          }

          if (filter.etatNotification.length > 0) {
            if (!filter.etatNotification.includes(item.notifications.etatCertification)) return false
          }

          if (filter.etatCertification != null && filter.etatCertification !== 'ALL') {
            if (filter.etatCertification === 'CERTIFIED' && item.certification_state !== 'CERTIFIED') return false
            if (filter.etatCertification !== 'CERTIFIED' && item.certification_state === 'CERTIFIED') return false
          }

          if (filter.engagement.length > 0) {
            if (!filter.engagement.includes(item.lastmixitestate) && item.lastmixitestate != null) return false
          }

          if (filter.pinned) {
            if (!numeroBioPinned.includes(Number(item.numeroBio))) return false
          }
...
async function getDashboardSummary (ocId, departements, anneeReferenceControle) {
  if (departements.length === 0) {
    fetchCustomersByOc({ siret: '', ocId, numeroBio: '', nom: '' })
  ...
  const numeroBios = records
    .filter((item) => departements.includes(item.departement))
    .map((r) => r.numeroBio)
  const { rows } = await pool.query(
    /* sql */`
      SELECT
  ...
  const countCertifiees = rows.find((r) => r.certifie === true)?.count ?? 0
  const countEnAttentes = rows.find((r) => r.certifie === false)?.count ?? 0
...
async function exportDataOcId (ocId) {
  try {
    const { rows } = await pool.query(
      ...
      COALESCE(SUM(CASE WHEN cp.conversion_niveau = 'CONV' THEN ST_Area(ST_Transform(cp.geometry, 2154)) / 10000 ELSE 0 END), 0) AS surface_conventionnel_ha,
      COALESCE(COUNT(CASE WHEN cp.conversion_niveau = 'CONV' THEN 1 ELSE NULL END), 0) AS nombre_conventionnel,
      COALESCE(SUM(CASE WHEN cp.conversion_niveau = 'C1' THEN ST_Area(ST_Transform(cp.geometry, 2154)) / 10000 ELSE 0 END), 0) AS surface_C1_ha,
      COALESCE(COUNT(CASE WHEN cp.conversion_niveau = 'C1' THEN 1 ELSE NULL END), 0) AS nombre_C1,
      COALESCE(SUM(CASE WHEN cp.conversion_niveau = 'C2' THEN ST_Area(ST_Transform(cp.geometry, 2154)) / 10000 ELSE 0 END), 0) AS surface_C2_ha,
      COALESCE(COUNT(CASE WHEN cp.conversion_niveau = 'C2' THEN 1 ELSE NULL END), 0) AS nombre_C2,
      COALESCE(SUM(CASE WHEN cp.conversion_niveau = 'C3' THEN ST_Area(ST_Transform(cp.geometry, 2154)) / 10000 ELSE 0 END), 0) AS surface_C3_ha,
      COALESCE(COUNT(CASE WHEN cp.conversion_niveau = 'C3' THEN 1 ELSE NULL END), 0) AS nombre_C3,
      COALESCE(SUM(CASE WHEN cp.conversion_niveau = 'AB' THEN ST_Area(ST_Transform(cp.geometry, 2154)) / 10000 ELSE 0 END), 0) AS surface_AB_ha,
      COALESCE(COUNT(CASE WHEN cp.conversion_niveau = 'AB' THEN 1 ELSE NULL END), 0) AS nombre_AB
      FROM cartobio_operators co
      LEFT JOIN cartobio_parcelles cp ON cp.record_id = co.record_id
      WHERE co.oc_id = $1
    ...
    for (const row of rows) {
      const data = await fetchOperatorByNumeroBio(row.numerobio)
      row.numeroclient = data.id
      row.siret = data.siret
      row.raisonSociale = data.nom
      row.codePostal = data.codePostal
      row.commune = data.commune
      const totalNonConventionnel = row.nombre_c1 + row.nombre_c2 + row.nombre_c3 + row.nombre_ab

      if (row.nombre_AB === row.nombre_parcelles) {
        row.mixite = '100% AB'
      } else if (totalNonConventionnel === row.nombre_parcelles) {
        row.mixite = '100% Engagé'
      } else if (row.nombre_conventionnel + totalNonConventionnel === row.nombre_parcelles) {
        row.mixite = 'Mixte'
      } else {
        row.mixite = '100% Conventionnel'
      }

      delete row.nombre_conventionnel
      delete row.nombre_c1
      delete row.nombre_c2
      delete row.nombre_c3
      delete row.nombre_ab
    }
    return rows
  } catch (error) {
    console.log(error)
