...
async function getOperatorLastRecord(numeroBio, { anneeAudit = null, statut = null } = {}) {
  ...
  const result = await pool.query(
    ...
    LIMIT 1`,
    [numeroBio, ...(anneeAudit ? [anneeAudit] : []), ...(statut ? [statut] : [])]
  );
...
async function* iterateOperatorLastRecords(ocId, { anneeAudit = null, statut = null } = {}) {
  ...
  `,
        [ocId, ...(anneeAudit ? [anneeAudit] : []), ...(statut ? [statut] : [])]
      )
    );
...
async function getRecord(recordId) {
  const result = await pool.query(
    /* sql */ `SELECT ${recordFields} FROM cartobio_operators WHERE record_id = $1 LIMIT 1`,
    [recordId]
  );
...
async function evvLookup({ numeroEvv }) {
  try {
    ...
    if (
      ...
      (error?.response?.statusCode === 404 || !Object.hasOwn(error, "code")) /* only for tests */
    ) {
...
async function evvParcellaire({ numeroEvv }) {
  ...
  return featureCollection(
    xml.parcellaire.listePcv.pcv.map((p) => {
      const id = getRandomFeatureId();
      const properties = {
        id,
        cadastre: [
          `${p.codeDepartement}${p.codeInseeCommune}${p.prefixe || "000"}${p.section.padStart("2", 0)}${p.plan.padStart(
            4,
            "0"
          )}`,
        ],
  ...
  audit_date: {
    fn(order) {
      ...
      return function sortByAuditDate(a, b) {
        return (
          collator.compare(a.audit_date ?? "", b.audit_date ?? "") *
  ...
  engagement_date: {
    fn(order) {
      ...
      return function sortByEngagementDate(a, b) {
        return (
          collator.compare(a.dateEngagement ?? "", b.dateEngagement ?? "") *
          (order === "asc" ? SORT.ASCENDING : SORT.DESCENDING)
        );
      };
    },
  ...
  nom: {
    fn(order) {
      ...
      return function sortByName(a, b) {
        return (
          collator.compare(a.denominationCourante || a.nom || "", b.denominationCourante || b.nom || "") *
          (order === "asc" ? SORT.ASCENDING : SORT.DESCENDING)
        );
      };
    },
  ...
  statut: {
    fn(order) {
      return function sortByStatut(a, b) {
        return (
          (STATUS_PRIORITY[a.certification_state] - STATUS_PRIORITY[b.certification_state]) *
          (order === "asc" ? SORT.ASCENDING : SORT.DESCENDING)
        );
      };
    },
      ...
      return /* sql */ `
        CASE certification_state
          ${Object.entries(STATUS_PRIORITY)
            .map(([key, value]) => `WHEN ${escapeLiteral(key)} THEN ${value} `)
            .join(" ")}
        END
...
async function searchControlBodyRecords({ ocId, input, page, sort, order }) {
  ...
  if (input) {
    const records = await fetchCustomersByOc({ siret, ocId, numeroBio, nom }).then((records) =>
      records.toSorted(recordSorts("fn", sort, order))
    );
    ...
    return Promise.all(
      allNumerobioResults.slice((pagination.page - 1) * PER_PAGE, pagination.page * PER_PAGE).map(async (numerobio) => {
        try {
...
async function updateAuditRecordState({ user, record }, patch) {
  ...
  try {
    ...
    if (e.code === "23505") {
      throw new InvalidRequestApiError("Un parcellaire ne peut pas avoir deux versions avec la même date d'audit.", {
        cause: e,
      });
    }
    ...
    const features = record.parcelles
      ...
      .map((parcelle) => {
        const id = !Number.isNaN(parseInt(String(parcelle.id), 10))
          ? parseInt(String(parcelle.id), 10)
          : getRandomFeatureId();
        ...
        try {
          conversionNiveau =
            parcelle.etatProduction && normalizeEtatProduction(parcelle.etatProduction, { strict: true });
        } catch (error) {
