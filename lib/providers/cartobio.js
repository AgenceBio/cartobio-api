...
const { addRecordData } = require('../outputs/operator.js')
...
async function searchControlBodyRecords ({ ocId, userId, input, page, filter, limit = 7 }) {
  ...
  const records = await Promise.all(
    [
      ...
      fetchCustomersByOcWithRecords({ siret, ocId, numeroBio, nom }, filter?.etatNotification, filter?.anneeReferenceCertification, filter?.departement, filter?.pinned, pinnedOperators)
    ]
  )
 ...
 * @return {Promise<any []>}
 */
async function searchForAutocomplete (ocId, input) {
  ...
  return records.filter((record) => {
    const recordNom = record.nom.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    const recordDenominationCourante = record.denominationCourante ? record.denominationCourante.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') : null
    const recordSiret = record.siret ? record.siret.replace(' ', '') : null

    return (nom && (recordNom.includes(nom) ||
    (recordDenominationCourante && recordDenominationCourante.includes(nom)))) ||
    (numeroBio && record.numeroBio.toString().includes(numeroBio)) ||
    (siret && recordSiret && recordSiret.includes(siret))
  }
  ).map((record) => ({ numeroBio: record.numeroBio, nom: record.nom, denominationCourante: record.denominationCourante, siret: record.siret }))
}
