...
async function * parseAPIParcellaireStream (stream, { organismeCertificateur }) {
  ...
  for await (const record of streamRecords) {
    ...
    if (!correctNumeroBio) {
      warnings.push({ numeroBio: String(record.numeroBio), message: 'Numéro bio pas à jour' })
    }
      ...
      .map(async (parcelle) => {
        ...
        try {
          ...
          if (!isValidPolygon.rows[0].valid) {
            warnings.push({ numeroBio: String(record.numeroBio), message: "Une des parcelle n'est pas valide" })
          }

          if (isValidPolygon.rows[0].empty) {
            warnings.push({ numeroBio: String(record.numeroBio), message: 'Une des parcelle est vide' })
          }
        ...
        warnings.push({ numeroBio: String(record.numeroBio), message: 'Parcelle en dehors des régions autorisées' })
        return resPolygon
      }))
