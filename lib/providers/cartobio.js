  ...
  engagement_date: {
    fn (order) {
      ...
      return function sortByEngagementDate (a, b) {
        ...
        console.log(aNotif.dateDemarrage)
  ...
  notifications: {
    fn (order) {
      ...
      return function sortByNotifications (a, b) {
        ...
        const bNotif = getNotification(b)
        console.log(order)
        return collator.compare(aNotif?.etatCertification ?? aNotif?.status ?? '', bNotif?.etatCertification ?? bNotif?.status ?? '') * (order === 'asc' ? SORT.ASCENDING : SORT.DESCENDING)
      }
    },
    psql () {
      // we are not supposed to enter this case
