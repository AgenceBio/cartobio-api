...
async function joinRecordParcelles (record, userId = null) {
  if (record && !record.parcelles) {
    ...
    SELECT cp.record_id, cp.id, cp.geometry, c.audit_date, c.numerobio
...
historique_cultures AS (
    SELECT
        ...
        r.cultures,
        r.conversion_niveau,
        co.audit_date AS annee_controle,
    ...
    AND co.audit_date < dp.audit_date
            ...
            (
                select jsonb_agg(
                    jsonb_build_object(
                        ...
                        'annee_controle', extract(year from hc.annee_controle)::int
                    )
                    ORDER BY hc.annee_controle DESC
                )
...
async function * parseAPIParcellaireStream (stream, { organismeCertificateur }) {
  ...
  for await (const record of streamRecords) {
    ...
    const features = await Promise.all(record.parcelles
      ...
      .map(async (parcelle) => {
        const id = parcelle.id ?? getRandomFeatureId()
