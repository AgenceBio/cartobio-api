...
async function createOrUpdateOperatorRecord (record, context = {}, customClient) {
  ...
  try {
    ...
    if (record.certification_state === CertificationState.CERTIFIED) {
      try {
  ...
  for (const feature of features) {
    ...
    const { rows } = await client.query(
      ...
      WHERE co.numerobio = $1
        ...
        AND ST_Equals(
          ST_ReducePrecision(ST_MakeValid(cp.geometry), 0.00001),
  ...
  try {
    ...
    if (state === CertificationState.CERTIFIED && record.certification_state !== CertificationState.CERTIFIED) {
      try {
        const emails = await fetchEmailForNumeroBio(record.numerobio)
        for (const utilisateur of emails.utilisateurs) {
          await sendCertificationComplete(record, utilisateur.email)
        }
        await pool.query(
        /* sql */`
        UPDATE cartobio_operators
