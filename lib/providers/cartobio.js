...
async function createOrUpdateOperatorRecord (data, context, customClient) {
  ...
  const { numeroBio, ocId, ocLabel, metadata, geojson, versionName, dateCertificationDebut, dateCertificationFin } = data
  ...
  try {
    result = await client.query(
      /* sql */`
        ...
        (numerobio, oc_id, oc_label, created_at, metadata, certification_state, certification_date_debut, certification_date_fin, audit_history, audit_notes, version_name)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, jsonb_build_array($9::jsonb), $10,
                COALESCE($11, 'Version crée le ' || to_char(now(), 'DD/MM/YYYY')))
        ON CONFLICT (numerobio, audit_date)
            DO UPDATE
            SET (oc_id, oc_label, updated_at, metadata, certification_state, certification_date_debut, certification_date_fin, audit_history, audit_date)
                    = ($2, $3, $4, $5, $6, $7, $8, (cartobio_operators.audit_history || coalesce($9, '[]')::jsonb), $10)
