    ...
    const features = await Promise.all(record.parcelles
      ...
      .map(async (parcelle) => {
        ...
        try {
          ...
          if (conversionNiveau !== null && [EtatProduction.C0, EtatProduction.C1, EtatProduction.C2, EtatProduction.C3, EtatProduction.CONV].includes(conversionNiveau) && parcelle.dateEngagement === null) {
            hasFeatureError = new Error('Champ date d engagement incorrect')
            return null
          }
        } catch (error) {
