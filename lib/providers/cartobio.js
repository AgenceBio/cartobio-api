        ...
        /* sql */`
          ...
          (
                ...
                WHERE numerobio = $2
                  AND NOT id ~ (
                      SELECT featureIds
                      FROM jsonb_to_recordset(cartobio_operators.audit_history)
                      AS x(featureIds text, type text, state text, date text)
                      WHERE
                        cartobio_operators.record_id = (SELECT record_id FROM previous_version)
                        AND type = 'FeatureCollectionCreation'
                        AND state = 'OPERATOR_DRAFT'
                      ORDER BY date DESC
                    )
                    AND cartobio_operators.record_id = (SELECT record_id FROM previous_version)
          )
          RETURNING *, ST_AsGeoJSON(geometry)::json as geometry
        `,
