  ...
  try {
    result = await client.query(
      /* sql */`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, jsonb_build_array($9::jsonb), $10, $11,
                COALESCE($12, 'Version crée le ' || to_char(now(), 'DD/MM/YYYY')))
        ON CONFLICT (numerobio, audit_date)
            DO UPDATE
            SET (oc_id, oc_label, updated_at, metadata, certification_state, certification_date_debut, certification_date_fin, audit_history, audit_date, audit_notes)
                    = ($2, $3, $4, $5, $6, $7, $8, (cartobio_operators.audit_history || coalesce($9, '[]')::jsonb), $10, $11)
  ...
  for await (const record of streamRecords) {
    ...
    const features = record.parcelles
      ...
      .map(parcelle => {
        ...
        const properties = {
          id,
          cultures: cultures.map(({ codeCPF: CPF, quantite, variete = '', dateSemis: date_semis = '', unite: unit }) => ({
            id: randomUUID(),
            CPF,
            date_semis,
