    ...
    .then((records) => {
      if (filter) {
        const temporisation = records.filter(item => {
          ...
          if (filter.etatCertification && filter.etatCertification !== 'ALL') {
            const estCertifie = item.certification_state === 'CERTIFIED'
            const filtreCertifie = filter.etatCertification === 'CERTIFIED'
            if (filtreCertifie) {
              // Si on filtre CERTIFIED, le parcellaire doit être certifié et les années doivent correspondre
              if (!estCertifie || item.annee_reference_controle !== filter.anneeReference) {
                return false
              }
            } else {
              // Si on filtre autre chose que CERTIFIED, on vérifie qu'il n'est pas certifié et on laisse passer si la date est undefined sinon on compare les années quand même
              if (estCertifie || (item.annee_reference_controle !== undefined && item.annee_reference_controle !== filter.anneeReference)) {
                return false
              }
            }
          }

          if (filter.statutParcellaire.length > 0 && !filter.statutParcellaire.includes(item.certification_state ?? 'NONE')) return false
