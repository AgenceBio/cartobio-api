...
async function prepareFeatures (features) {
  ...
  return Promise.all(sorted.map(async (feature) => {
    ...
    if (fixResult.statut === 'UNCORRECTABLE') {
      throw new InvalidRequestApiError(`Une géométrie n'est pas corrigeable (${id})`)
    }
    ...
    const { rows } = await client.query(
      /* sql */`
        ...
        DO UPDATE SET
          ...
          statut_import_geom = COALESCE($18, cartobio_parcelles.statut_import_geom)
