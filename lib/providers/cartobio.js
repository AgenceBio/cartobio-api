...
async function searchControlBodyRecords ({ ocId, userId, input, page, filter }) {
  ...
  const records = await Promise.all(
    [
      ...
      fetchCustomersByOcWithRecords({ siret, ocId, numeroBio, nom }, filter?.etatNotification)
    ]
  )
    .then(([numeroBioPinned, records]) => {
      if (filter) {
        const temporisation = records.filter(item => {
          ...
          if (filter.statutParcellaire && filter.statutParcellaire.length > 0 && !filter.statutParcellaire.includes(item.certification_state ?? 'NONE')) return false
