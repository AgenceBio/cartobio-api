 ...
 */
async function createOrUpdateOperatorRecord (operatorId, data, context, client) {
    ...
    */
    result = await (client || pool).query(/* sql */
      ` WITH merged_features AS (
            ...
            FROM cartobio_operators, jsonb_array_elements($7::jsonb->'features') AS new_features
        ...
        ON CONFLICT (numerobio)
            ...
            SET (oc_id, oc_label, updated_at, metadata, parcelles, certification_state, audit_history)
               = (SELECT $3, $4, $5, $6, jsonb_build_object(
                        'type', 'FeatureCollection',
                        'features', merged_features.features
                    ), $8, (cartobio_operators.audit_history || coalesce($9, '[]')::jsonb)
...
async function parcellaireStreamToDb (stream, organismeCertificateur) {
  ...
  try {
    for await (const [numeroBio, data, error] of generator) {
      ...
      await createOrUpdateOperatorRecord(null, {
        numeroBio,
        ocId: data.ocId,
