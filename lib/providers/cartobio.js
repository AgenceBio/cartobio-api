...
const { fetchCustomersByOc, fetchCustomersByOcWithRecords, fetchUserOperators, fetchCustomersByAdmin, fetchEmailForNumeroBio } = require('./agence-bio.js')
...
async function createOrUpdateOperatorRecord (record, context = {}, customClient) {
  ...
  try {
    ...
    if (context?.copyParcellesData && context?.previousRecordId) {
      const addedParcelles = await client.query(
        /* sql */`
          ...
          (
                SELECT $1, cp.id, cp.geometry, cp.commune, cp.cultures, cp.conversion_niveau, cp.engagement_date, NULL as auditeur_notes,
                    cp.name, cp.reference_cadastre, cp.code_culture_pac, cp.code_precision_pac
                FROM cartobio_operators co
                CROSS JOIN jsonb_to_recordset(co.audit_history)
                    AS ah("featureIds" text, type text, state text, date text)
                JOIN cartobio_parcelles cp ON co.record_id = cp.record_id
                WHERE
                    ah.type = 'FeatureCollectionCreation'
                    AND ah.state = 'OPERATOR_DRAFT'
                    AND NOT ah."featureIds" ~ cp.id
                    AND NOT EXISTS (
                      SELECT 1 FROM cartobio_parcelles cp_base
                      WHERE cp_base.record_id = $1
                      AND cp_base.deleted_at IS NULL
                      AND (
                          ST_Area(ST_Intersection(cp_base.geometry, cp.geometry)) / ST_Area(cp_base.geometry) > 0.5
                          OR
                          ST_Area(ST_Intersection(cp_base.geometry, cp.geometry)) / ST_Area(cp.geometry) > 0.5
                        )
                    )
                    AND co.record_id = $3
                    AND co.numerobio = $2
    ...
    if (isNewParcellaire.rowCount === 0 && record.certification_state === CertificationState.CERTIFIED) {
      try {
      ...
      const { rows: partialUpdateRows } = await client.query(
        /* sql */`
          ...
          SET (commune, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
               ...
               reference_cadastre, code_culture_pac, code_precision_pac, etranger, statut_import_geom) =
  ...
  try {
    ...
    await client.query(
      /* sql */`
        ...
        SET (geometry, commune, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
             annotations, updated, name, numero_pacage, numero_ilot_pac, numero_parcelle_pac,
             reference_cadastre, code_culture_pac, code_precision_pac, attente_pac) =
                (coalesce($3, cartobio_parcelles.geometry), coalesce($4, cartobio_parcelles.commune),
                 ...
                 coalesce($16, cartobio_parcelles.code_culture_pac),
                 coalesce($17, cartobio_parcelles.code_precision_pac),
                 coalesce($18, cartobio_parcelles.attente_pac))
        WHERE record_id = $1 AND id = $2`,
      [
        record.record_id,
        featureId,
  ...
  try {
    ...
    if (state === CertificationState.CERTIFIED && record.certification_state !== CertificationState.CERTIFIED) {
      try {
        ...
        console.error(`Erreur lors de l'envoi de la notification de certification pour le record ${record.record_id}`)
      }
    }
  } catch (e) {
    if (e.code === '23505') {
...
async function fixGeometry (geometry) {
  const isValidRows = await pool.query(
    `
