  ...
  try {
    ...
    for (let feature of record.parcelles.features) {
      ...
      if (!feature.geometry) {
        const { rows: partialUpdateRows } = await client.query(
          ...
          UPDATE cartobio_parcelles
              SET (cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
              ...
              (coalesce($3, cartobio_parcelles.cultures),
               coalesce($4, cartobio_parcelles.conversion_niveau),
               nullif(coalesce($5::text, cartobio_parcelles.engagement_date::text), '')::date,
               coalesce($6, cartobio_parcelles.commentaire), coalesce($7, cartobio_parcelles.auditeur_notes),
               coalesce($8, cartobio_parcelles.annotations), now(), coalesce($9, cartobio_parcelles.name),
               coalesce($10, cartobio_parcelles.numero_pacage), coalesce($11, cartobio_parcelles.numero_ilot_pac),
               coalesce($12, cartobio_parcelles.numero_parcelle_pac),
               coalesce($13, cartobio_parcelles.reference_cadastre),
               coalesce($14, cartobio_parcelles.code_culture_pac),
               coalesce($15, cartobio_parcelles.code_precision_pac))
          ...
          [
            ...
            /*  $3 */ feature.properties.cultures ? JSON.stringify(feature.properties.cultures) : null,
            /*  $4 */ feature.properties.conversion_niveau,
            /*  $5 */ feature.properties.engagement_date,
            /*  $6 */ feature.properties.commentaires,
            /*  $7 */ feature.properties.auditeur_notes,
            /*  $8 */ feature.properties.annotations ? JSON.stringify(feature.properties.annotations) : null,
            /* $9 */ feature.properties.NOM,
            /* $10 */ feature.properties.PACAGE,
            /* $11 */ feature.properties.NUMERO_I,
            /* $12 */ feature.properties.NUMERO_P,
            /* $13 */ feature.properties.cadastre,
            /* $14 */ feature.properties.TYPE,
            /* $15 */ feature.properties.CODE_VAR
          ]
        )
      ...
      const { rows } = await client.query(
        /* sql */`
          WITH previous_version AS (
              ...
              WHERE numerobio = $15
                ...
                AND cartobio_operators.record_id = $19
          ...
          previous_variete AS (
              ...
              FROM previous_version, jsonb_array_elements($4::jsonb) AS new_cultures
          ...
          merged_cultures AS (
              ...
              FROM jsonb_array_elements($4::jsonb) AS new_cultures
              LEFT JOIN previous_variete ON previous_variete.CPF = new_cultures->>'CPF'
              WHERE $18
            )
          INSERT INTO cartobio_parcelles
          (record_id, id, geometry, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
           annotations, created, updated, name, numero_pacage, numero_ilot_pac, numero_parcelle_pac,
           reference_cadastre, code_culture_pac, code_precision_pac)
          VALUES ($1, $2, $3, coalesce((SELECT cultures FROM merged_cultures), $4, '[]'::jsonb),
                  coalesce(NULLIF($5, 'AB?'), (SELECT conversion_niveau FROM previous_version), $5),
                  nullif(coalesce($6::text, (SELECT engagement_date FROM previous_version)::text), '')::date, $7,
                  coalesce($8,(SELECT auditeur_notes FROM previous_version)),
                  coalesce($9, '[]'::jsonb),
                  now(), now(), coalesce(NULLIF($10, ''), (SELECT name FROM previous_version)), $11, $12, $13, $14, $16, $17)
          ON CONFLICT (record_id, id)
              DO UPDATE
              SET (geometry, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
                   annotations, updated, name, numero_pacage, numero_ilot_pac, numero_parcelle_pac,
                   reference_cadastre, code_culture_pac, code_precision_pac) =
                      (coalesce($3, cartobio_parcelles.geometry),
                       coalesce($4, cartobio_parcelles.cultures), coalesce($5, cartobio_parcelles.conversion_niveau),
                       nullif(coalesce($6::text, cartobio_parcelles.engagement_date::text), '')::date,
                       coalesce($7, cartobio_parcelles.commentaire),
                       coalesce($8, cartobio_parcelles.auditeur_notes), coalesce($9, cartobio_parcelles.annotations),
                       now(), coalesce($10, cartobio_parcelles.name), coalesce($11, cartobio_parcelles.numero_pacage),
                       coalesce($12, cartobio_parcelles.numero_ilot_pac), coalesce($13, cartobio_parcelles.numero_parcelle_pac),
                       coalesce($14, cartobio_parcelles.reference_cadastre), coalesce($16, cartobio_parcelles.code_culture_pac),
        ...
        [
          ...
          /* $15 */ result.rows.at(0).numerobio,
          /* $16 */ feature.properties.TYPE,
          /* $17 */ feature.properties.CODE_VAR,
          /* $18 */ Boolean(context?.copyParcellesData),
          /* $19 */ context?.previousRecordId || randomUUID()
        ]
      )
    ...
    if (context?.copyParcellesData && context?.previousRecordId) {
      const addedParcelles = await client.query(
        /* sql */`
          INSERT INTO cartobio_parcelles
          (record_id, id, geometry, cultures, conversion_niveau, engagement_date, auditeur_notes,
            name, reference_cadastre, code_culture_pac, code_precision_pac)
          (
                SELECT $1, id, geometry, cultures, conversion_niveau, engagement_date, auditeur_notes,
                    name, reference_cadastre, code_culture_pac, code_precision_pac
  ...
  try {
    ...
    await client.query(
      /* sql */`
        ...
        SET (geometry, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
             ...
             reference_cadastre, code_culture_pac, code_precision_pac) =
                (coalesce($3, cartobio_parcelles.geometry),
                 coalesce($4, cartobio_parcelles.cultures), coalesce($5, cartobio_parcelles.conversion_niveau),
                 nullif(coalesce($6::text, cartobio_parcelles.engagement_date::text), '')::date, coalesce($7, cartobio_parcelles.commentaire),
                 coalesce($8, cartobio_parcelles.auditeur_notes),
                 coalesce($9, cartobio_parcelles.annotations), now(), coalesce($10, cartobio_parcelles.name),
                 coalesce($11, cartobio_parcelles.numero_pacage),
                 coalesce($12, cartobio_parcelles.numero_ilot_pac),
      ...
      [
        ...
        /* $2 */ featureId,
        /* $3 */ geometry,
        /* $4 */ properties.cultures ? JSON.stringify(properties.cultures) : null,
        /* $5 */ properties.conversion_niveau,
        /* $6 */ properties.engagement_date,
        /* $7 */ properties.commentaires,
        /* $8 */ properties.auditeur_notes,
        /* $9 */ properties.annotations ? JSON.stringify(properties.annotations) : null,
        /* $10 */ properties.NOM,
        /* $11 */ properties.PACAGE,
        /* $12 */ properties.NUMERO_I,
        /* $13 */ properties.NUMERO_P,
        /* $14 */ properties.cadastre,
        /* $15 */ properties.TYPE,
        /* $16 */ properties.CODE_VAR
      ])
    ...
    await client.query(
      /* sql */`
        ...
        (record_id, id, geometry, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
         annotations, created, updated, name, numero_pacage, numero_ilot_pac, numero_parcelle_pac,
         reference_cadastre, code_culture_pac, code_precision_pac)
        VALUES ($1, $2, $3, coalesce($4, '[]')::jsonb, $5, $6, $7, $8, coalesce($9, '[]')::jsonb, now(), now(),
                $10, $11, $12, $13, $14, $15, $16)`,
      [
        /* $1 */ record.record_id,
        /* $2 */ newId,
        /* $3 */ feature.geometry,
        /* $4 */ JSON.stringify(feature.properties.cultures ?? []),
        /* $5 */ feature.properties.conversion_niveau,
        /* $6 */ feature.properties.engagement_date,
        /* $7 */ feature.properties.commentaires,
        /* $8 */ feature.properties.auditeur_notes,
        /* $9 */ JSON.stringify(feature.properties.annotations ?? []),
        /* $10 */ feature.properties.NOM,
        /* $11 */ feature.properties.PACAGE,
        /* $12 */ feature.properties.NUMERO_I,
        /* $13 */ feature.properties.NUMERO_P,
...
async function addDividFeature (user, record, operator, features, info) {
  ...
  try {
    if (features.features.length === 1) {
      ...
      /* sql */`
        UPDATE cartobio_parcelles
        SET geometry = $3, cultures = $4, conversion_niveau = $5, engagement_date = $6, commentaire = $7, auditeur_notes = $8,
        annotations = $9, updated = now(), name = $10, numero_pacage = $11, numero_ilot_pac = $12, numero_parcelle_pac = $13,
        reference_cadastre = $14, code_culture_pac = $15, code_precision_pac = $16
        WHERE record_id = $1 AND id = $2`,
        [
          /* $1 */ info.recordId,
          /* $2 */ info.featureId,
          /* $3 */ features.features[0].geometry,
          /* $4 */ JSON.stringify(features.features[0].properties.cultures),
          /* $5 */ features.features[0].properties.conversion_niveau,
          /* $6 */ features.features[0].properties.engagement_date,
          /* $7 */ features.features[0].properties.commentaires,
          /* $8 */ features.features[0].properties.auditeur_notes,
          /* $9 */ JSON.stringify(features.features[0].properties.annotations),
          /* $10 */ features.features[0].properties.NOM,
          /* $11 */ features.features[0].properties.PACAGE,
          /* $12 */ features.features[0].properties.NUMERO_I,
          /* $13 */ features.features[0].properties.NUMERO_P,
          /* $14 */ features.features[0].properties.cadastre,
          /* $15 */ features.features[0].properties.TYPE,
          /* $16 */ features.features[0].properties.CODE_VAR
        ])
      const { rows: updatedRows } = await setOperatorUpdatedAt(client, record, historyEntry, user, operator)
      ...
      for (const feature of features.features) {
        ...
        await client.query(
          /* sql */`
            ...
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, now(), now(), $10, $11, $12, $13, $14, $15, $16, $17)`,
          [
            /* $1 */ record.record_id,
            /* $2 */ newId,
            /* $3 */ feature.geometry,
...
async function exportDataOcId (ocId, filter, userId) {
  try {
    const operators = await searchControlBodyRecords({ ocId, userId, input: filter.input, page: 1, limit: Infinity, filter: filter.filterForSearch })
      .then(e => {
        e.records = e.records.sort((a, b) => {
          if (a.numeroBio === b.numeroBio) {
          ...
          return +a.numeroBio - +b.numeroBio
        })
        return e
      })
    const ids = operators.records.map(({ numeroBio }) => String(numeroBio))
    let results = []
    let index = 0
    const limit = 10000
    while (index < ids.length) {
      ...
      const { rows } = await pool.query(
        /* sql */ `
            ...
            co.metadata->>'campagne' as campagne,
            co.created_at,
            co.updated_at,
            co.mixite as mixite,
    ...
    for (const row of results) {
      while (+operators.records[j].numeroBio !== +row.numerobio) {
        j++
      }
      const operator = operators.records[j]
