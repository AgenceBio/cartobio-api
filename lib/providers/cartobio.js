...
async function getOperator ({ operatorId }) {
  const [result, operator] = await Promise.all([
    pool.query('SELECT record_id, certification_state, created_at, updated_at, parcelles, metadata FROM cartobio_operators WHERE operator_id = $1 LIMIT 1', [operatorId]),
