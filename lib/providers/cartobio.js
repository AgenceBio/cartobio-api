...
const { fetchCustomersByOc, fetchCustomersByOcWithRecords, fetchUserOperators, fetchCustomersByAdmin } = require('./agence-bio.js')
...
async function createOrUpdateOperatorRecord (record, context = {}, customClient) {
  ...
  try {
    ...
    for (let feature of record.parcelles.features) {
      ...
      } else if (fixResult.statut === 'CORRIGE') {
        feature = polygon(fixResult.geometry.coordinates, feature.properties, { id })
        feature.properties.statut_import_geom = 'CORRIGE'
      } else if (fixResult.statut === 'ACCEPTENONCORRIGE') {
        feature.properties.statut_import_geom = 'ACCEPTENONCORRIGE'
      } else {
        feature.properties.statut_import_geom = 'ACCEPTE'
      }

      const etranger = await isEtranger(feature.geometry)
      feature.properties.etranger = etranger
      if (etranger) feature.properties.COMMUNE = undefined

      if (!etranger) {
        if (!feature.properties.COMMUNE || !(await communeExists(feature.geometry, feature.properties.COMMUNE))) {
          feature.properties.COMMUNE = await findCommune(
            feature.geometry, feature.properties.COMMUNE
          )
        }
      }
      ...
      if (!feature.geometry) {
        const { rows: partialUpdateRows } = await client.query(
          ...
          UPDATE cartobio_parcelles
              ...
              reference_cadastre, code_culture_pac, code_precision_pac,statut_import_geom) =
              ...
              SET (geometry, commune, cultures, conversion_niveau, engagement_date, commentaire, auditeur_notes,
                   ...
                   reference_cadastre, code_culture_pac, code_precision_pac, statut_import_geom) =
                      (coalesce($3, cartobio_parcelles.geometry), coalesce($4, cartobio_parcelles.commune),
                       ...
                       coalesce($18, cartobio_parcelles.code_precision_pac),coalesce($21, cartobio_parcelles.statut_import_geom))
          RETURNING *, ST_AsGeoJSON(geometry)::json as geometry`,
        [
          /*  $1 */ result.rows.at(0).record_id,
          /*  $2 */ feature.id || getRandomFeatureId(),
          ...
          /* $21 */ feature.properties.statut_import_geom || 'ACCEPTE'
        ]
      )
      if (rows.at(0)) {
...
async function updateAuditRecordState ({ user, record, operator }, patch) {
  ...
  if (state) {
    ...
    if (state === CertificationState.CERTIFIED && !columns.includes('mixite')) {
      await sendCertificationComplete(record, 'test@test.fr')
...
async function communeExists (geometry, code) {
  const result = await pool.query(
    ...
    SELECT EXISTS(
      ...
      WHERE code = $1
      AND (
        ST_Within(geometry, ST_SetSRID(ST_GeomFromGeoJSON($2),4236))
