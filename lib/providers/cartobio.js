...
async function searchControlBodyRecords ({ ocId, input, page, sort, order }) {
  ...
  if (input) {
    const records = await fetchCustomersByOc({ siret, ocId, numeroBio, nom }, page, PER_PAGE)

    const pagination = {
      page,
      total: records.total,
      page_max: Math.max(Math.ceil(records.total / PER_PAGE), 1)
    }

    return {
      pagination,
      records: records.operateurs
    }
