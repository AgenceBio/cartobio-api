...
const { fetchCustomersByOc, fetchCustomersByOcWithRecords, fetchUserOperators } = require('./agence-bio.js')
  ...
  try {
    ...
    for (let feature of record.parcelles.features) {
      ...
      if (fixResult.uncorrectable) {
        ...
        throw new InvalidRequestApiError(`Une géométrie n'est pas corrigable (${featureLabel})`)
      }
      if (fixResult.fixed) {
        feature.geometry = polygon(fixResult.geometry.coordinates, properties, {
          id
        })
      }
      ...
      feature.properties.COMMUNE = undefined

      if (!etranger) {
        if (!feature.properties.COMMUNE || !(await communeExists(feature.properties.COMMUNE))) {
          feature.properties.COMMUNE = await findCommune(
            feature.properties.geometry, feature.properties.COMMUNE
          )
        }
      }
      ...
      if (!feature.geometry) {
        const { rows: partialUpdateRows } = await client.query(
          ...
          [
            ...
            /* $16 */ feature.properties.etranger || false
          ]
        )
      ...
      const { rows } = await client.query(
        /* sql */`
          WITH previous_version AS (
              SELECT conversion_niveau, engagement_date, name, cultures
    ...
    // TODO : get code erreur import / implix
...
/**
 * Creation de parcelles liés a d'autres
 *
 * @param {CartoBioUser} user - The user performing the operation.
 * @param {NormalizedRecord} record - The record containing the feature.
 * @param {AgenceBioNormalizedOperator} operator
 * @param {CartoBioFeature[]} features - The features to add or divide.
 * @param {Number[]} from - ID des parcelles d'origine
...
async function createFeaturesFromOther (user, record, operator, features, from) {
  ...
  try {
    historyEntry = createNewEvent(
      EventType.FEATURE_CREATE,
      { features: features, description: 'Parcelles ajoutées' },
      { user, record }
    )
    await client.query(
      /* sql */`
...
`
  ...
  return Promise.all(rows.map(async row => normalizeRecord(await joinRecordParcelles(row))))
}
...
async function getRecord (recordId, user = null) {
  ...
  return normalizeRecord(await joinRecordParcelles(record, user?.id))
}
...
async function markFeatureControlled (recordId, id, userId) {
  await pool.query(
    /* sql */`
      INSERT INTO parcelles_controlees (record_id, id, user_id)
      VALUES ($1, $2, $3)
      ON CONFLICT (record_id, id, user_id) DO NOTHING`,
    [recordId, id, userId]
  )
}
...
async function markFeatureUncontrolled (recordId, id, userId) {
  await pool.query(
    /* sql */`
      DELETE FROM parcelles_controlees
      WHERE record_id = $1 AND id = $2 AND user_id = $3`,
    [recordId, id, userId]
  )
}
...
async function getImportPAC (numeroBio) {
  const { rows } = await pool.query(
    /* sql */`
      SELECT size, nb_parcelles, pacage, imported, record
      FROM import_pac
      WHERE numerobio = $1`
    , [numeroBio]
  )
...
async function getFeaturesFromRecordId (recordId) {
  const { rows } = await pool.query(
    /* sql */`
...
async function fixGeometry (geometry) {
  ...
  if (row.diff_ha >= 1 || row.diff_pct >= 1) {
    return { uncorrectable: true }
  }

  return { fixed: true, geometry: row.geometry }
}
...
async function communeExists (geometry, code) {
  const result = await pool.query(
    `
    SELECT EXISTS(SELECT 1 FROM communes WHERE code = $1) AS exists
  `,
    [code]
  )
...
async function findCommune (geometry, code) {
  const result = await pool.query(
    `
      WITH dept AS (
        SELECT CASE
          WHEN $2::text IS NULL THEN NULL
          WHEN LEFT($2::text, 2) = '97' THEN LEFT($2::text, 3)
          ELSE LEFT($2::text, 2)
        END AS code_dept
      ),
      pre_selection AS (
        SELECT code FROM communes, dept
        WHERE dept.code_dept IS NOT NULL
          AND LEFT(code, LENGTH(dept.code_dept)) = dept.code_dept
          AND ST_Intersects(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), geometry)
          AND ST_Area(ST_Intersection(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), geometry), true)
            / ST_Area(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), true) >= 0.5
        ORDER BY
          ST_Area(ST_Intersection(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), geometry), true) DESC,
          ST_Distance(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), geometry) ASC
        LIMIT 1
      ),
      fallback AS (
        SELECT code FROM communes
        WHERE NOT EXISTS (SELECT 1 FROM pre_selection)
        ORDER BY
          ST_Area(ST_Intersection(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), geometry), true) DESC,
          ST_Distance(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326), geometry) ASC
        LIMIT 1
      )
      SELECT code FROM pre_selection
      UNION ALL
      SELECT code FROM fallback
      LIMIT 1
  `,
    [JSON.stringify(geometry), code]
  )
