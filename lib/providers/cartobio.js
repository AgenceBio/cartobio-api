...
async function prepareFeatures (features) {
  ...
  return Promise.all(sorted.map(async (feature) => {
 ...
 *
 * @param {Array<any>} newCultures
 * @param {Array<any>} oldCultures
 * @returns {Array<any>}
 */
function mergeCulturesExact (newCultures = [], oldCultures = []) {
  ...
  for (const newC of newCultures) {
    ...
    if (varietes.length) {
      map.set(cpf, [...new Set(varietes)].join('; '))
    }
  }
  ...
  await client.query(
    ...
    FROM (
      SELECT
        co.record_id,
        CASE
          WHEN COUNT(*) FILTER (WHERE cp.conversion_iveau = 'AB') = COUNT(*)
            THEN 'AB'
          WHEN COUNT(*) FILTER (WHERE cp.conversion_niveau IN ('C1','C2','C3','AB')) = COUNT(*)
            THEN 'ABCONV'
          WHEN COUNT(*) FILTER (WHERE cp.conversion_iveau = 'CONV') > 0
            THEN 'MIXTE'
          ELSE NULL
        END AS mixite
      FROM cartobio_operators co
