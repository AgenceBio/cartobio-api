 ...
 */
async function createOperatorRecord ({ operatorId, decodedToken }, { numeroBio, ocId, ocLabel, geojson, metadata, historyEvent }) {
  /** @type {HistoryEntry} */
  const historyEntry = createNewEvent(
    EventType.FEATURE_COLLECTION_CREATE,
    { features: geojson.features },
    { state: CertificationState.OPERATOR_DRAFT, metadata },
    { decodedToken }
  )

  const { state: certificationState } = historyEntry

  let result = await pool.query(/* sql */`SELECT record_id, audit_history
    FROM cartobio_operators
    WHERE operator_id = $1 AND certification_state IS NULL`,
  [operatorId])

  // in case we delete + imported a data again
  if (result.rows.length) {
    const { record_id: recordId, audit_history: auditHistory } = result.rows.at(0)
    ...
    [recordId, numeroBio, ocId, ocLabel, 'now', metadata, geojson, certificationState, JSON.stringify(auditHistory)])
  // import from scratch
  } else {
 ...
 * @returns {OperatorRecord}
 */
async function patchFeatureCollection ({ recordId, decodedToken }, features) {
  const result = await pool.query('SELECT record_id, parcelles, certification_state FROM cartobio_operators WHERE record_id = $1 LIMIT 1', [recordId])
  ...
  const { rows: updatedRows } = await pool.query(/* sql */`UPDATE cartobio_operators SET
    updated_at = now(), parcelles = $2::jsonb, audit_history = (audit_history || $3::jsonb)
    WHERE record_id = $1
    RETURNING ${recordFields}`,
  [recordId, parcelles, historyEntry])
 ...
 * @returns {OperatorRecord}
 */
async function updateFeatureProperties ({ recordId, featureId, decodedToken }, { properties }) {
  const result = await pool.query('SELECT record_id, parcelles, certification_state FROM cartobio_operators WHERE record_id = $1 LIMIT 1', [recordId])
  const [record] = result.rows
  ...
  const { rows: updatedRows } = await pool.query(/* sql */`UPDATE cartobio_operators
    SET updated_at = now(), parcelles = $2, audit_history = (audit_history || $3::jsonb)
    WHERE record_id = $1
    RETURNING ${recordFields}`,
  [
    recordId,
...
async function addRecordFeature ({ recordId, decodedToken }, feature) {
  ...
  const { rows: updatedRows } = await pool.query(/* sql */`UPDATE cartobio_operators
    SET updated_at = now(), parcelles['features'] = (parcelles->'features' || $2::jsonb), audit_history = (audit_history || $3::jsonb)
    WHERE record_id = $1
    RETURNING ${recordFields}`,
  [
    recordId,
...
async function deleteRecord ({ recordId, decodedToken }) {
  /** @type {HistoryEntry} */
  const historyEntry = createNewEvent(
    EventType.FEATURE_COLLECTION_DELETE,
    { },
    {
      description: 'Réinitialisation du parcellaire'
    },
    { decodedToken }
  )

  const result = await pool.query(/* sql */`UPDATE cartobio_operators
    SET updated_at = now(), parcelles = NULL, certification_date_debut = NULL, certification_date_fin = NULL, certification_state = NULL, metadata = '{}'::jsonb, audit_history = (audit_history || $2::jsonb)
    WHERE record_id = $1
    RETURNING ${recordFields}`,
  [recordId, historyEntry])
