 ...
 * @return {Promise<any[]>}
 */
async function haveImportPAC (numeroBio) {
  const { rows } = await pool.query(
    /* sql */`
      SELECT size, nb_parcelles, numeropacage
      FROM import_pac
      WHERE numerobio = $1 AND imported = false`
    , [numeroBio]
  )
  return rows
}
 ...
 * @return {Promise<any[]>}
 */
async function fetchDataImportPAC (numeroBio) {
  const { rows } = await pool.query(
    /* sql */`
    SELECT record
    FROM import_pac
