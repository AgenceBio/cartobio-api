    ...
    <div class="container">
      <header>
        <div class="header-left">
          ...
          <div>
            <span class="name-operator"> ${currentOperator.nom ? currentOperator.nom : '-'}</span>
             <div class="badge">
              <svg
                width="12"
                height="12"
              ...
              <div>CERTIFIÉ | ${currentOperator.annee_reference_controle ? currentOperator.annee_reference_controle : '-'}</div>
            </div>
            </div>
            </div>
          </div>
        ...
        <div class="left">
          <div class="ligne-header">
            ...
            <p>
              ...
              ${currentOperator.adressesOperateurs[0].lieu ? currentOperator.adressesOperateurs[0].lieu : '-'}
              <br/>
              ${currentOperator.adressesOperateurs[0].codePostal ? currentOperator.adressesOperateurs[0].codePostal : '-'}
              ${currentOperator.adressesOperateurs[0].ville ? currentOperator.adressesOperateurs[0].ville : '-'}
            </p>
          </div>
          <div class="ligne-header">
            <p>
              <span class="title-header">Année réf. contrôle&nbsp;:</span>
              ${currentOperator.annee_reference_controle ? currentOperator.annee_reference_controle : '-'}
            </p>
            <p>
          ...
          <div class="ligne-header">
            <p>
              ...
              ${currentOperator.certification_date_debut ? formatDate(currentOperator.certification_date_debut) : '-'}
            </p>
            <p>
              <span class="title-header">Fin de validité du certificat&nbsp;:</span>
              ${currentOperator.certification_date_fin ? formatDate(currentOperator.certification_date_fin) : '-'}
            </p>
          </div>
          <div class="ligne-header">
            <p>
...
`
    ...
    // eslint-disable-next-line no-undef
      ...
      // @ts-ignore
      // eslint-disable-next-line no-undef
      document.querySelector('#numeropacage').textContent = currentOperator.numeros_pacage ?? currentOperator.numeroPacage ?? '-'
      // @ts-ignore
      // eslint-disable-next-line no-undef
    ...
    await page2.pdf({
      path: `pdf/${numeroBio}-${recordId}-${dateTraitement}-0.pdf`,
      format: 'A4',
      landscape: true,
      printBackground: true,
      pageRanges: '1',
  ...
  const pdfFiles = [`pdf/${numeroBio}-${recordId}-${dateTraitement}-0.pdf`, `pdf/${numeroBio}-${recordId}-${dateTraitement}-1.pdf`, `pdf/${numeroBio}-${recordId}-${dateTraitement}-2.pdf`]
