      ...
      <div class="header-top">
        <div class="flex">
          <span class="name-operator">
            ${currentOperator.nom ? currentOperator.nom : '-'}</span
          >
          <div class="badge">
            <svg
              width="12"
...
: '-'}
        ...
        <div>
          ...
          <span>${currentOperator.numeroBio}</span>
        </div>
      </div>
          ...
          <div class="ligne-header">
            <p>
              <span class="title-header"
                >Début de validité du certificat&nbsp;:</span
              >
...
: '-'}
            ...
            <p>
              <span class="title-header"
                >Fin de validité du certificat&nbsp;:</span
              >
...
: '-'}
          ...
          <div class="ligne-header">
            <p>
              ...
              currentOperator.mixite === 'AB'
? '100% AB'
              : currentOperator.mixite === 'ABCONV'
? 'AB/En conversion'
              : currentOperator.mixite === 'MIXTE' ? 'Mixte' : '-'}
            </p>
            <p>
...
`
  ...
  const pdfFiles = [`pdf/${numeroBio}-${recordId}-${dateTraitement}-1.pdf`, `pdf/${numeroBio}-${recordId}-${dateTraitement}-2.pdf`]

  // Génération PDF avec détails
  const pdf = [await mergeAndAddFooter(pdfFiles, currentOperator.nom, !pac)]
  if (pac) {
    // Génération PAC sans détails
    pdf.push(await mergeAndAddFooter([`pdf/${numeroBio}-${recordId}-${dateTraitement}-1.pdf`], currentOperator.nom, true))
  }
  return pdf
}
