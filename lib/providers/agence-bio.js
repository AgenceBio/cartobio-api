 ...
 */
async function fetchCustomersByOcWithRecords ({ ocId: oc, siret = '', numeroBio = '', nom = '' }, statusNotifications = [], anneeReferenceControle = null, departements = [], pinned = false, pinnedOperators = null) {
  ...
  if (anneeReferenceControle) {
    operators = operators.filter((item) => item.notifications && item.notifications.dateDemarrage && new Date(item.notifications.dateDemarrage).getFullYear() <= anneeReferenceControle)
  }

  if (departements && departements.length > 0) {
    operators = operators.filter((item) => item.departement && departements.includes(item.departement))
  }

  if (pinned === true && Array.isArray(pinnedOperators)) {
