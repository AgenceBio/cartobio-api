...
async function getUserProfileFromSSOToken (accessToken) {
  ...
  // AgenceBio exposes the info as `ext`, whereas Notification token is directly embedded
  const { userId } = decodedToken.ext ?? decodedToken

  const userProfile = await get(`${config.get('notifications.endpoint')}/api/users/${userId}`, {
