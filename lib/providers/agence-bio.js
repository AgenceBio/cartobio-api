...
async function operatorLookup ({ q }) {
  ...
  const result = await get(`${NOTIFICATIONS_AB_ENDPOINT}/api/operateurs`, {
    headers: {
      Origin,
  ...
  return (result?.items ?? []).map(({ id, nom, denominationCourante, siret, numeroPacage, email, dateEngagement, datePremierEngagement, adressesOperateurs }) => ({
    id,
    nom,
