 ...
 */
async function getOperatorsByOc ({ serviceToken, oc, siret = '', numeroBio = '', pacage = '', nom = '' }) {
 ...
 */
async function fetchCustomersByOc ({ ocId: oc, siret = '', numeroBio = '', nom = '' }) {
  const operators = await getOperatorsByOc({ serviceToken, siret, oc, numeroBio, nom })
...
/**
 * Returns operators for dashboard of a given user
 * @param {number[]} numeriobios
 * @return {Promise<{operators: AgenceBioNormalizedOperator[]}>}
 */
async function fetchUserOperatorsForDashboard (numeriobios) {
  const operators = await Promise.all(numeriobios.map((numeroBio) => fetchOperatorByNumeroBio(numeroBio.toString())))

  return {
    operators: operators.map((o) => ({ ...o, epingle: true }))
  }
}
...
module.exports = {
  ...
  fetchUserOperatorsForDashboard,
  fetchUserOperators,
  fetchCustomersByOc,
  getUserProfileById,
