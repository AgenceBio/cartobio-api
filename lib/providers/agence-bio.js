 ...
 * @returns {Promise<{ total: number, operateurs: AgenceBioNormalizedOperator[] }>}
 */
async function _getOperatorsByOc ({ serviceToken, oc, siret = '', numeroBio = '', pacage = '', nom = '' }, page, limit) {
  const data = await get(`${config.get('notifications.endpoint')}/api/oc/${oc}/operateurs?nb=${limit}&page=${page}`, {
    headers: {
  ...
  return { total: data.total, operateurs: data.operateurs.map(normalizeOperator) }
}
 ...
 * @param {number?} limit
 * @param {number?} offset
 * @return {Promise<{nbTotal: number, operators: AgenceBioNormalizedOperator[]}>}
 */
async function fetchUserOperators (userId, limit, offset) {
  const { nbTotal, items } = await get(`${config.get('notifications.endpoint')}/api/utilisateur/${userId}/operateurs`, {
    headers: {
      Authorization: serviceToken,
      Origin
    },
  ...
  return {
    nbTotal,
    operators: items.map(normalizeOperator)
  }
}

/**
 * @param  {{numeroBio: String?, ocId: number, nom: string?, siret: string?}} params
* @returns {Promise<{ total: number, operateurs: AgenceBioNormalizedOperatorWithRecord[] }>}
 */
async function fetchCustomersByOc ({ ocId: oc, siret = '', numeroBio = '', nom = '' }, page, limit) {
  const data = await getOperatorsByOc({ serviceToken, siret, oc, numeroBio, nom }, page, limit)
  const operateursWithRecords = await populateWithRecords(data.operateurs)
  return { total: data.total, operateurs: operateursWithRecords }
}
