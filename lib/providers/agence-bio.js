...
async function operatorLookup ({ q }) {
  ...
  const items = await Promise.all([
    get(`${NOTIFICATIONS_AB_ENDPOINT}/api/operateurs`, {
      headers,
      searchParams: { q, activites }
    }).json(),
    get(`${NOTIFICATIONS_AB_ENDPOINT}/portail/v3/operateurs`, {
      headers,
      searchParams: { nom: q, activites, statusNotification: 'A VALIDER' }
    }).json()
  ]).then(results => {
    return results.reduce((items, result) => result?.items ? items.concat(result.items) : items, [])
  })

  return items.map(({ id, nom, denominationCourante, siret, numeroBio, numeroPacage, certificats, email, dateEngagement, datePremierEngagement, adressesOperateurs }) => ({
    id,
    nom,
