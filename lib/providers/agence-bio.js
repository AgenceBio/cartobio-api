  ...
  for await (const record of stream.pipe(JSONStream.parse([true]))) {
    const features = record.parcelles
      ...
      .map(parcelle => ({
        ...
        geometry: {
          type: 'Polygon',
          coordinates: JSON.parse(parcelle.geom)
        },
        properties: {
          CAMPAGNE: record.anneeReferenceControle,
          culture_cpf: parcelle.culture.at(0).codeCPF,
          /* eslint-disable-next-line camelcase */
          TYPE: fromCodeCpf(parcelle.culture.at(0).codeCPF).cultures_pac.find(({ requires_precision }) => requires_precision === false)?.code,
          NUMERO_I: parcelle.numeroIlot,
          NUMERO_P: parcelle.numeroParcelle,
          PACAGE: record.numeroPacage,
          conversion_niveau: parcelle.etatProduction
        }
      }))
