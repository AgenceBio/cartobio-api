 ...
 * @param {{ organismeCertificateur: OrganismeCertificateur }} stream
 * @return {Iterable<Object>}
 */
async function * parseAPIParcellaireStream (stream, { organismeCertificateur }) {
  /**
   * @type {ParcellaireApiRecord[]}
   */
  ...
  for await (const record of streamRecords) {
    const features = record.parcelles
      ...
      .map(parcelle => {
        ...
        const firstCulture = cultures.at(0)
        const { numeroIlot, numeroParcelle } = parsePacDetailsFromComment(parcelle.commentaire)
        ...
        return {
          ...
          properties: {
            id,
            remote_id: parcelle.id,
            CAMPAGNE: record.anneeReferenceControle,
            CPF: firstCulture.codeCPF,
            culture_variete: firstCulture.variete,
            cultures: cultures.map(({ codeCPF, quantite, variete }) => ({
              CPF: codeCPF,
              surface: parseFloat(quantite),
              variete
            })),
            NUMERO_I: parcelle.numeroIlot ? String(parcelle.numeroIlot) : numeroIlot,
            NUMERO_P: parcelle.numeroParcelle ? String(parseInt(parcelle.numeroParcelle, 10)) : numeroParcelle,
            PACAGE: String(record.numeroPacage),
            conversion_niveau: normalizeEtatProduction(parcelle.etatProduction),
            engagement_date: parcelle.dateEngagement,
            audit_notes: parcelle.commentaire
          }
        }
      })
