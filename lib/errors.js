...
const ExpiredCredentialsApiError = createError(
  "EXPIRED_CREDENTIALS",
  "Accès refusé : ce jeton n'est plus valide (%s).",
  401
);
...
const PreconditionFailedApiError = createError(
  "PRECONDITION_FAILED",
...
const errorHandler = (error, request, reply) => {
  ...
  return reply.send(
    new Error("Erreur serveur. Nous avons été informés et résoudrons ceci au plus vite.", {
      cause: error.message,
    })
  );
};
