...
function FastifyErrorHandler ({ sentryClient }) {
  ...
  return function errorHandler (error, request, reply) {
    ...
    sentryClient && sentryClient.captureException(error)

    this.log.error({
      sentToSentry: Boolean(sentryClient),
      message: error.message,
      stack: error.stack,
      remoteRequest: error.cause && error.cause.response ? error.cause.response.body : null
    })
