...
function isAfterCertificationState (referenceState) {
  ...
  return function isAfter (lookup) {
    return stateValues.indexOf(lookup) > lookupIndex
  }
}

const isAfterAudited = isAfterCertificationState(CertificationState.AUDITED)
...
const EventTypeRules = [
  ...
  ({ eventType: type, record }) => type === EventType.FEATURE_CREATE && isAfterAudited(record.certification_state),
  // [AUDITED] Suppression de parcelle
  ({ eventType: type, record }) => type === EventType.FEATURE_DELETE && isAfterAudited(record.certification_state),
  // [*] Terminer l’audit
  ...
  ({ eventType: type, record }) => type === EventType.FEATURE_UPDATE && isAfterAudited(record.certification_state),
  ({ eventType: type, record }) => type === EventType.FEATURE_COLLECTION_UPDATE && isAfterAudited(record.certification_state)
]

function shouldCreateHistoryEvent ({ record, eventType, decodedToken, state = null }) {
  return EventTypeRules.some(rule => rule({ record, eventType, decodedToken, state }))
}
