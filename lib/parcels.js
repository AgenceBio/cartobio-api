...
async function getOperatorSummary ({ ocId }) {
  ...
  const features = customers
    // we keep active and geolocated operators
    // once we have broader data, we can remove the 'numeroPacage' from the filter
    .filter(({ numeroPacage, departementId, active }) => numeroPacage && departementId && active)
    .map(({ nom, denominationCourante, numeroBio, numeroPacage, departementId, active }) => {
      const { code: departement, centroid } = getDepartementFromId(departementId)

      return point(
        centroid,
        {
          nom: nom || denominationCourante || '',
          numerobio: numeroBio,
          pacage: numeroPacage,
          departement,
          active
