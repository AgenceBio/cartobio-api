...
describe("dashboard controller", () => {
  ...
  it("applique les valeurs de pagination par défaut", async () => {
    mockedRepo.getTableauBilanRepo.mockResolvedValue({
      data: [],
      meta: { total: 0, page: 1, limit: 20 },
    });
    const reply = makeReply();

    await getTableauBilan({ query: {} }, reply);
  ...
  it("enveloppe les KPI dans data", async () => {
    ...
    expect(mockedRepo.getGeneralKpi).toHaveBeenCalledWith({
      from: "2026-01-01",
      to: "2026-01-31",
    });
  ...
  it("enveloppe l’historique des imports dans data", async () => {
    ...
    await getHistoriqueImports({ query: { page: "3", limit: "5" } }, reply);
  ...
  it("transmet les filtres du tableau des erreurs", async () => {
    ...
    await getTableauErreurs(
      { query: { from: "2026-02-01", recherche: "ABC" } },
      reply
    );
  ...
  it("transmet les critères de l’historique parcellaire", async () => {
    ...
    expect(
      mockedRepo.getHistoriqueSpecificParcellaireRepo
  ...
  it("enveloppe les top anomalies simples dans data", async () => {
    ...
    await getTopAnomalies(
      { query: { from: "2026-01-01", to: "2026-01-07" } },
      reply
    );

    expect(mockedRepo.getTopAnomaliesRepo).toHaveBeenCalledWith({
      from: "2026-01-01",
      to: "2026-01-07",
    });
    expect(reply.send).toHaveBeenCalledWith({ data });
  ...
  it("renvoie directement les anomalies groupées et les répétitions", async () => {
    ...
    await getTopAnomaliesGrouped(
      { query: { from: "2026-01-01", to: "2026-01-07" } },
      groupedReply
    );
    await getRepetErreurs(
      { query: { from: "2026-01-01", to: "2026-01-07" } },
      suspectsReply
    );
