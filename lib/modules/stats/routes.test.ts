...
describe('parcellaireRoutes', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    jest.mocked(protectedWithToken).mockReturnValue({ preHandler: [] } as any)
    jest.mocked(mergeSchemas).mockReturnValue({} as any)
  })

  it('enregistre toutes les routes GET avec le même schéma de protection', async () => {
    const get = jest.fn()
    await parcellaireRoutes({ get } as any, {} as any
    )
    ...
    expect(get).toHaveBeenNthCalledWith(6, '/top-anomalies-grouped', expect.any(Object), controller.getTopAnomaliesGrouped)
