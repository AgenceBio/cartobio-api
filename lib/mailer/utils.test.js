...
jest.mock('../providers/api-parcellaire.js', () => ({
  getStatsByOrganisme: jest.fn().mockResolvedValue({
    totalrecu: 10,
    totalacceptes: 8,
    totalrefuses: 2
  })
  ...
  test('sendCertificationComplete envoie le bon sujet', async () => {
    ...
    expect(sendMail).toHaveBeenCalledWith(
      expect.objectContaining({
        to: 'test@example.com',
        subject: expect.stringContaining(String(mockRecord.annee_reference_controle))
      })
    )
  })
