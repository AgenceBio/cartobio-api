...
async function populateWithRecords(operators) {
  ...
  const { rows } = await pool.query(
    /* sql */ `SELECT certification_state, audit_date, certification_date_debut, created_at, numerobio, metadata, record_id, updated_at
  ...
  return operators.map((agenceBioOperatorData) => {
    const { metadata, ...record } = rows.find(
      ({ numerobio }) => numerobio === String(agenceBioOperatorData.numeroBio)
    ) || { metadata: {} };
...
function normalizeOperator(operator) {
  ...
  return {
    ...
    adressesOperateurs: adressesOperateurs
      ...
      .map(({ lat, long, codeCommune }) => ({ lat, long, codeCommune })),
