 ...
 */
async function getFilterData (operators, states, mixite) {
  ...
  if (mixite === true) {
    ...
    if (!states) {
      select.push('last_certified_parcelle.numerobio')
    }
  }
...
SELECT
  ${select.join(',')}
FROM ${states === true ? 'all_parcellaire' : 'last_certified_parcelle'}
  ${states === true && mixite === true ? 'LEFT JOIN last_certified_parcelle ON all_parcellaire.numerobio = last_certified_parcelle.numerobio' : ''}
    `,
    [ids])

  return operators.map(agenceBioOperatorData => {
    const data = rows.find(({ numerobio }) => numerobio === String(agenceBioOperatorData.numeroBio))
 ...
 * @return {boolean}
 */
function applyOperatorTextSearch (record, siret, numeroBio, nom) {
  ...
  nom = nom ? nom.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') : ''

  if (!siret && !numeroBio && !nom) {
    return true
  }

  if (nom && (recordNom.includes(nom) || recordDenominationCourante.includes(nom))) {
    return true
  }
  if (numeroBio && record.numeroBio.toString().includes(numeroBio)) {
    return true
  }
  if (siret && recordSiret && recordSiret.includes(siret)) {
    return true
  }
