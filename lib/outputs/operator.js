...
function getNotification (notifications) {
    ...
    Object.keys(groupedNotifications).forEach((key) => {
      ...
      group.sort((a, b) => {
        if (a.etatCertification === b.etatCertification) {
          ...
          // TODO : nouveau if || a.etatCertification === 'ARRÊTÉE'
  ...
  } else if (withoutOldOC.length > 0) {
    const lastNotification = withoutOldOC.sort((a, b) => new Date(b.changementDate).getTime() - new Date(a.changementDate).getTime())[0]
