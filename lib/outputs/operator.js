...
SELECT
    all_parcellaire.numerobio,
    all_parcellaire.states
FROM all_parcellaire
LEFT JOIN last_certified_parcelle ON all_parcellaire.numerobio = last_certified_parcelle.numerobio
    `,
    [ids])
 ...
 * Populate third party information with our very own operators data
 *
 * @param  {AgenceBioNormalizedOperatorWithRecord} operateur An array of objects fetched from the Agence Bio Notifications portal
 * @return {Promise<AgenceBioNormalizedOperatorWithRecord>}
 */
...
ORDER BY combined_results.numerobio, COALESCE(combined_results.certification_date_debut, combined_results.audit_date, combined_results.created_at) DESC;
  ...
  return {
    ...operateur,
    metadata: { ...metadata, ...operateur.metadata },
    ...record,
