...
function decodeUserToken({ keys }) {
  ...
  return async function decodeTokenMiddleware(request) {
     ...
     */
    const { user, error } = await Promise.allSettled(verifiers.map((fn) => fn(token))).then((results) => ({
      // @ts-ignore
  ...
  return async function verifyAgenceBioTokenMiddleware(request) {
    ...
    if (foundToken && !foundToken.startsWith("Bearer ")) {
      try {
        ...
        if (error.code) {
          throw new UnauthorizedApiError("un jeton d'API Agence Bio valide est nécessaire pour accéder à ce contenu.", {
            cause: error,
          });
        } else {
          throw new BadGatewayApiError("Le portail d'authentification de l'Agence Bio n'a pas pu être contacté.", {
            cause: error,
          });
        }
      }
...
function compareOperatorOutputAndToken(accessor) {
  ...
  return async function compareOperatorOutputAndTokenMiddleware(request, _, payload) {
    ...
    if (
      operator &&
      Object.hasOwn(operator.organismeCertificateur, "id") &&
      userOc &&
      userOc.id !== operator.organismeCertificateur.id
    ) {
      throw new UnauthorizedApiError("vous n'êtes pas autorisé·e à accéder à ce contenu avec vos identifiants.");
    }
