...
const { checkOcToken, fetchCertificationBody, fetchOperatorByNumeroBio } = require('./providers/agence-bio.js')
const { ExpiredCredentialsApiError, InvalidCredentialsApiError, InvalidRequestApiError, NotFoundApiError, UnauthorizedApiError } = require('./errors.js')
/**
...
function verifyAgenceBioToken () {
  if (config.get('env') === 'dev' && config.get('environment') !== 'production' && config.get('dangerousServerCertificationBodyId')) {
    console.warn('Le bypass de jeton OC est activé')

    return async function bypassAgenceBioTokenMiddleware (request) {
      const ocId = config.get('dangerousServerCertificationBodyId')

      request.organismeCertificateur = (await fetchCertificationBody()).find(({ id }) => id === ocId)
    }
  }
    ...
    if (foundToken && !foundToken.startsWith('Bearer ')) {
      try {
        ...
        if (error.code) {
          ...
          throw new InvalidRequestApiError('erreur inconnue', { cause: error })
        }
      }
    }
  }
