...
const { checkOcToken, fetchUserOperators } = require('./providers/agence-bio.js')
const { ExpiredCredentialsApiError, InvalidCredentialsApiError, NotFoundApiError, UnauthorizedApiError, ForbiddenApiError } = require('./errors.js')
...
const { isRevoked } = require('./auth/revocation')
/**
...
function decodeUserToken ({ keys }) {
  ...
  return async function decodeTokenMiddleware (request) {
    ...
    if (user && await isRevoked(token)) {
      throw new InvalidCredentialsApiError('Le token a été révoquer')
    }
...
function enforceOperator ({ param }) {
  ...
  return async function enforceOperatorMiddleware (request) {
    ...
    try {
      if (user !== null && String(user.mainGroup.id) === '4') {
        const op = (await fetchUserOperators(user.id)).operators.find(e => String(e.numeroBio) === numeroBio)
        if (!op) {
          throw new ForbiddenApiError('Accès interdit')
        }
...
function enforceRecordAndOperator ({ queryFn, param }) {
  ...
  return async function enforceRecordAndOperatorMiddleware (request) {
    const id = request.params[param]
    const record = await queryFn(id, request.user)
    ...
    // Un opérateur n'a pas le droit de voir les audit_notes qui lui sont attribues
    if (request.operator && record && String(request.user?.mainGroup.id) === '4') {
      record.audit_notes = null
    }
...
function haveRightToUpdate () {
  /**
   * @param {FastifyRequest & { body: { properties?: Object.<string, any> } & Object.<string, any> }} request
   */
  return async function haveRightToUpdateAuditMiddleware (request) {
    const { user, record, body: patch } = request

    if (!user) return

    const restrictedFields = ['NOM', 'commentaires']

    const hasOnlyAllowedChanges = patch?.properties
      ? Object.keys(patch.properties).every(
        (key) =>
        // @ts-ignore
          restrictedFields.includes(key) || patch.properties[key] === record?.properties?.[key]
      )
      : Object.keys(patch ?? {}).every(
        (key) => {
          return restrictedFields.includes(key) || patch[key] === record?.[key]
        }
      )

    if (
      String(user.mainGroup.id) === '4' &&
      record?.certification_state !== 'OPERATOR_DRAFT' &&
      !hasOnlyAllowedChanges
    ) {
      throw new UnauthorizedApiError("vous n'êtes pas autorisé·e à modifier à ce contenu avec vos identifiants.")
    }

    if (
      String(user.mainGroup.id) === '8' &&
...
function compareOperatorOutputAndToken (accessor) {
  ...
  return async function compareOperatorOutputAndTokenMiddleware (request, _, payload) {
    ...
    if (operator && Object.hasOwn(operator.organismeCertificateur, 'id') && userOc && userOc.id === operator.organismeCertificateur.id) {
      ...
      if (record && record.oc_id !== userOc.id) {
        ...
        record.parcelles = {
          ...
          features: record.parcelles.features.map((p) => {
            p.properties.auditeur_notes = null
      ...
      if (!operator.notifications || operator.notifications.etatCertification !== 'NON ENGAGEE') {
        return payload
      }
    }
...
function checkCertification (request) {
  if (!request.user || !request.user.organismeCertificateur) {
    return
  }

  const array = request.operator?.certificats ?? request.operator?.notifications
  const currentStatut = array.etatCertification

  if (currentStatut === 'ARRETEE' || request.user.organismeCertificateur.id !== request.operator?.organismeCertificateur?.id) {
    throw new UnauthorizedApiError("vous n'êtes pas autorisé·e à accéder à ce contenu avec vos identifiants.")
  }
}
...
function enforceAdminRole () {
  return async function (request, reply) {
    if (!(String(request.user?.mainGroup.id) === '1')) {
      reply.code(403)
      throw new UnauthorizedApiError("vous n'êtes pas autorisé·e à accéder à ce contenu avec vos identifiants.")
    }
  }
