...
const { decodeUserToken, enforceAdminRole, enforceRecord, enforceRecordAndOperator, enforceOperator, verifyAgenceBioToken, enforceAnyFastifyDecorator, compareOperatorOutputAndToken, checkCertification, haveRightToUpdate } = require('../middlewares.js')
 ...
 * @returns {CartobioExtendedRouteShorthandOptions}
 */
function protectedWithToken ({ oc = false, cartobio = true, admin = false } = {}) {
  ...
  if (admin) {
    const name = 'user'
    if (!requiredDecorators.includes(name)) requiredDecorators.push(name)
    root.schema.tags.push(TAGS.ADMIN)
    root.schema.security.push({ bearerAuth: [] })
      root.onRequest.push(decodeUserToken({
        keys: [
          // this one if for the tokens we emit with this application
    ...
    root.onRequest.push(enforceAdminRole())
  }
...
const dashboardSummarySchema = {
  schema: {
    body: {
      type: 'object',
      required: ['departements', 'anneeReferenceControle'],
      properties: {
        departements: {
          type: 'array',
          items: {
