...
const { decodeToken, enforceToken, compareToken, trackRequest, compareOperatorOutputAndToken, decodeTokenWithPK } = require('../middlewares.js')
...
const protectedRouteOptions = {
  ...
  preValidation: [
    decodeToken({ name: 'decodedToken', JWT_SECRET: config.get('jwtSecret') }),
...
const notificationRouteOptions = {
  ...
  preValidation: [
    decodeTokenWithPK({ name: 'notificationToken', PUBLIC_KEY: config.get('notifications.publicKey') }),
    enforceToken({ name: 'notificationToken' })
  ]
}
