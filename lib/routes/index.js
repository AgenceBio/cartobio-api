 ...
 * HOOKS
 */
function protectedWithToken ({ oc = false, cartobio = false, mattermost = false } = { oc: false, cartobio: true, mattermost: false }) {
  const requiredDecorators = []
  const root = {
    schema: { tags: [], securitySchemes: [] },
    preValidation: []
  }

  if (oc) {
    requiredDecorators.push('organismeCertificateur')
    root.schema.tags.push('Organisme de certification')
    root.schema.securitySchemes.push({ bearerAuth: [] })
    root.preValidation.push(verifyAgenceBioToken())
  }

  if (cartobio) {
    requiredDecorators.push('decodedToken')
    root.schema.tags.push(internalSchema.schema.tags.at(0))
    root.schema.securitySchemes.push({ agenceBioAuthorizationAuth: [] })
    root.preValidation.push(decodeToken({
      name: 'decodedToken',
      keys: [
  ...
  if (mattermost) {
    root.schema.securitySchemes.push({ tokenHeaderAuth: [] })
    root.preValidation.push(compareToken({
  ...
  if (requiredDecorators.length) {
    root.preValidation.push(enforceAnyFastifyDecorator(requiredDecorators))
  }
