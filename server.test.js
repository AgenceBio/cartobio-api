...
const OTHER_USER_DOC_AUTH_TOKEN = sign({
  id: 2,
  prenom: "Tania",
  nom: "Ouest",
  test: true,
  organismeCertificateur: otherFakeOc,
});
...
const apiRecordExpect = expectDeepCloseTo(recordToApi(normalizeRecord({ parcelles, ...record })));
...
describe("GET /api/v2/test", () => {
  test("fails when JWT is missing, or invalid", async () => {
    const response = await request(app.server).get("/api/v2/test").type("json");
...
describe("GET /api/v2/audits/:recordId", () => {
  ...
  test("it works with a matching user token", async () => {
    ...
    expectedRecord.parcelles.features.forEach((f) => delete f.properties.createdAt);
...
describe("POST /api/v2/convert/telepac/geojson", () => {
  ...
  test.each(["telepac-multi-2024v4.xml", "telepac-multi-2024v3.xml"])(
    "it converts a L93 multi-feature XML %s to WGS84 GeoJSON",
    (filename) => {
      return request(app.server)
        .post("/api/v2/convert/telepac/geojson")
        .type("json")
  ...
  test.each(["telepac-single-2024v3.xml", "telepac-single-2024v4.xml"])(
    "it converts a L93 single-feature XML %s to WGS84 GeoJSON",
    (filename) => {
      return request(app.server)
        .post("/api/v2/convert/telepac/geojson")
        .type("json")
  ...
  test.each(["geofolia-parcelles.zip", "anygeo/anygeo-test.zip"])(
    "it fails when sending a non-recognized format (%s)",
    async (filepath) => {
      const response = await request(app.server)
        .post("/api/v2/convert/telepac/geojson")
        .type("json")
...
describe("HEAD/GET /api/v2/import/geofolia/:numeroBio", () => {
  ...
  describe("HEAD /api/v2/import/geofolia/:numeroBio", () => {
    ...
    test("it fails without auth (head)", async () => {
      const response = await request(app.server).head("/api/v2/import/geofolia/99999").type("json");
  ...
  describe("GET /api/v2/import/geofolia/:numeroBio", () => {
    ...
    test("it fails without auth (get)", async () => {
      const response = await request(app.server).get("/api/v2/import/geofolia/99999").type("json");
...
describe("POST /api/v2/convert/anygeo/geojson", () => {
  test("it fails without auth (get)", async () => {
    const response = await request(app.server).post("/api/v2/convert/anygeo/geojson").type("json");
  ...
  test.each([
    "anygeo-test.kml",
    "anygeo-test.geojson",
    "anygeo-test-cartobio.json",
    "anygeo-test.gpkg",
    "anygeo-test.gpkg.zip",
    "anygeo-test.kmz",
    "anygeo-test.zip",
  ])("it responds to %s with 2 features", (filename) => {
    return request(app.server)
      ...
      .then((response) => {
        expect(response.status).toEqual(200);
        expect(area(response.body)).toBeCloseTo(457013.2 + 391240.7, 1);
  ...
  test.each(["geofolia-parcelles.zip", "../../package.json", "anygeo/anygeo-test.xlsx"])(
    "it fails with non-anygeo file %s",
    async (filepath) => {
      const response = await request(app.server)
        .post("/api/v2/convert/anygeo/geojson")
        .attach("archive", `test/fixtures/${filepath}`)
...
describe("PATCH /api/v2/audits/:recordId/parcelles", () => {
  ...
  test("it updates only the patched properties of the features", async () => {
    ...
    patchedRecordExpectation.parcelles.features.forEach((parcelle) => {
...
describe("GET /api/v2/import/evv/:numeroEvv+:numeroBio", () => {
  ...
  const xmlParcellaire = fs.readFileSync(join(__dirname, "lib", "providers", "__fixtures__/cvi-parcellaire.xml"), {
    encoding: "utf8",
  });
...
describe("GET /api/v2/certification/parcellaires", () => {
  ...
  const expectedResult = records.reverse().reduce((acc, record) => {
    record = expectDeepCloseTo(recordToApi(normalizeRecord({ parcelles, ...record })));
  ...
  test("it should return a list of parcellaire", async () => {
    const res = await request(app.server).get("/api/v2/certification/parcellaires").set("Authorization", fakeOcToken);
  ...
  test("it should return a 401 without auth", async () => {
    const res = await request(app.server).get("/api/v2/certification/parcellaires");
...
describe("POST /api/v2/certification/parcelles", () => {
  ...
  test("it maintains the value of optional fields", async () => {
    ...
    await request(app.server).post("/api/v2/certification/parcelles").set("Authorization", fakeOcToken).send([payload]);

    const { rows } = await db.query("SELECT * FROM cartobio_operators WHERE numerobio = $1 AND audit_date = $2", [
      "99999",
      "2022-05-01",
    ]);
    ...
    const { rows: parcelles } = await db.query("SELECT * FROM cartobio_parcelles WHERE record_id = $1", [
      rows[0].record_id,
    ]);
